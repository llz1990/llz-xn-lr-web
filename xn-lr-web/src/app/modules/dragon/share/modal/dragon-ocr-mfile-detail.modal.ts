...
export class DragonOcrMfileDetailModalComponent {
    ...
    public isShowOcrAlert:boolean = false;
    public ocrClass:string = '';
    ...
    public handleSubmit() {
        ...
        if(this.qrsType && QrsTypeEnum[this.qrsType] === 'vanke'){
            for(let key in this.mainForm.value) {
                if(key === 'contractNum' && (this.params.contractNum !== this.mainForm.value.contractNum || this.params.debtUnit !== this.mainForm.value.debtUnit ||
                    this.params.debtUnitAccount !== this.mainForm.value.debtUnitAccount || this.params.debtUnitBank !== this.mainForm.value.debtUnitBank)){
                    param['modelType'] = 2;
