...
export class FinancingSupplier6Flow implements IFlowCustom {
    ...
    preSubmit(svrConfig: any, formValue: any): Observable<any> {
        try {
            ...
            let payables = formValue.payables;
            let payable = parseFloat(formValue.payables.replace(reg, '')).toFixed(2) || 0;
            let contractFile = formValue.contractFile;
            let transferAll = JSON.parse(formValue.invoiceFile);
            let transferAmount = this.computeSum(transferAll.filter(v =>
    ...
    private calcHeadquarters(array: any[]): string {
        if (array.length) {
            const find = array.find(x => x.procedureId === 'operate');
            if (find) {
