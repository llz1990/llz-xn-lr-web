 ...
 * 1.0                 wangqing          合同盖章检测         2019-08-28
 * **********************************************************************
 */
...
export class DragonPdfSignModalComponent implements OnInit {
    ...
    open(params: any): Observable<any> {
        ...
        this.showOrHide();
        this.xn.dragon.post('/contract/contract/json', {
            id: params.id,
            secret: params.secret
        }).subscribe(json => {
            try {
                const ret = this.ocx.SZCA_LoadFromString(json.data);
                this.currentContractPreSignMd5 = md5(json.data); // 计算md5
                if (ret !== 0) {
    ...
    public onSign() {
        ...
        setTimeout(() => {
            ...
            this.xn.dragon.post3('/contract/upload', {
                id: this.params.id,
    ...
    private handlePageConfig(): string {
        let pageCount = this.ocx.SZCA_GetPageCount();
        let retPage = [];

        // 如果this.params.config中有页码是负数的，则转换为实际页码
        let pages = this.params.config.position.split('#');
        for (let page of pages) {
            let items = page.split(',');
            let pageNum = parseInt(items[items.length - 1], 0);
            if (pageNum < 0) {
