...
import { ButtonConfigModel, SubTabListOutputModel, TabConfigModel, TabListOutputModel } from '../../config/list-config-model';
import { CheckersOutputModel } from '../../config/checkers';
import { ApiProxyEnum, SubTabEnum, } from '../common/index-tab.config';
...
import { AvengerPdfSignModalComponent } from '../shared/components/modal/pdf-sign-modal.component';
import { AvengerapprovalfreeStyleComponent } from '../shared/components/modal/avenger-approval-freeStyle.modal';
import { AvengerChangeAccountComponent } from '../shared/components/modal/avenger-approval-changeAccount.modal';
import { AvengerResultCompareComponent } from '../shared/components/modal/avenger-approval-resultCompare.modal';
import { AvengerExportListModalComponent } from '../shared/components/modal/export-list-modal.component';
import * as _ from 'lodash';
import { SignContractsModalComponent } from '../../public/form/bank/sign-contracts-modal.component';
import { AvengerIrecordComponent } from '../shared/components/modal/avenger-irecordInfo.modal';
import { DragonFinancingContractModalComponent } from '../../modules/dragon/share/modal/dragon-asign-contract.modal';
import { DragonPdfSignModalComponent } from '../../modules/dragon/share/modal/pdf-sign-modal.component';
import { AvengerMfilesViewModalComponent } from '../shared/components/modal/mfiles-view-modal.component';
...
export class AvengerListComponent implements OnInit {
    ...
    public unfinishedCount: number = 0;
    ...
    watchApproval_msg: string = '';
    pushApproval_msg: string = '';
    ...
    ngOnInit(): void {
        this.rooterChange = this.routers.events.subscribe((data) => {
            //路由导航结束之后处理
    ...
    */
    public initData(paramTabValue: string, init?: boolean, ) {
        ...
        if (this.tabConfig.title.includes('审批放款列表') && (this.defaultValue === 'A' || this.defaultValue === 'C')) {
            // if (isFirst !== undefined && isFirst === true) {
            //     this.localStorageService.setCacheValue('startStatus', { startStatus: true });
            // }
            this.xn.api.post('/tool/is_jindie', {}).subscribe(x => {
                if (x.data) {
                    this.is_jindie = x.data.isOpen;
                    if (this.is_jindie === 1) {
                        this.tabConfig.title = '审批放款列表(已开启金蝶云审批)';
                    } else {
                        this.tabConfig.title = '审批放款列表(线下人工审批)';
                    }
                }
            });
        }
        this.subDefaultValue = 'DOING'; // 重置子标签默认
        if (this.defaultValue === 'C' && this.tabConfig.title.includes('审批放款列表')) {
            this.pushApproval(1);
            clearInterval(this.timed);
        } else if (this.defaultValue === 'A' && this.tabConfig.title.includes('审批放款列表')) {
            clearInterval(this.timedpush);
    ...
    public onPage(e?: { page: number, first?: number, pageSize?: number, pageCount?: number }, types?: number) {
        if (this.tabConfig.title.includes('审批放款列表') && this.defaultValue === 'A') {
    ...
    watchApproval(type?: number) {
        this.timed = window.setInterval(() => {
            let that = this;
            that.xn.avenger.post2('/jd/approval_progress', {
            }).subscribe(data => {
                if (data.ret !== 0) {
                    clearInterval(this.timed);
                    // this.xn.msgBox.open(false, '');
                    // this.xn.msgBox.showChangeText(`${data.msg}`);
                    if (this.subDefaultValue === 'SPECIAL') {
                        this.cancelSpecial(this.subDefaultValue);
                    } else {
                        this.getList(this.currentParams);
                    ...
                    if (data.data.isCompleted === true && type && type === 1) {
                        ...
                        && type === undefined
                        && this.localStorageService.caCheMap.get('isstartStatus').startStatus === true) {
                        if (this.subDefaultValue === 'SPECIAL') {
                            this.cancelSpecial(this.subDefaultValue);
                        } else {
                            this.getList(this.currentParams);
                        ...
                        this.watchApproval_msg = ``;
                        this.localStorageService.setCacheValue('isstartStatus', { startStatus: false });
                        clearInterval(this.timed);
    ...
    pushApproval(type?: number) {
        this.timedpush = window.setInterval(() => {
            let that = this;
            that.xn.avenger.post2('/jd/push_progress', {
            }).subscribe(data => {
                if (data.ret !== 0) {
                    clearInterval(this.timedpush);
                    ...
                    if (data.data.isCompleted === true && type && type === 1) {
                        ...
                        && type === undefined
                        && this.localStorageService.caCheMap.get('ispushStatus').startStatus === true) {
                        this.getList(this.currentParams);
                        this.pushApproval_msg = ``;
                        this.localStorageService.setCacheValue('ispushStatus', { startStatus: false });
                        clearInterval(this.timedpush);
                    } else if (data.data.isCompleted === false) {
    ...
    public showContract(paramContractInfo, type: string, proxy: number) {
        if ((proxy === 52 || proxy === 53) && type !== 'receivetable') {
    ...
    public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        ...
        if (paramBtnOperate.operate === 'initiate_examination') {    //
            // let freezeOne = this.selectedItems.map(x => x.freezeOne);
            // if (freezeOne.includes(1)) {
            //     this.xn.msgBox.open(false, '此交易已被冻结，无法发起审批');
            //     return;
            // }
            ...
            XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                if (v === null) {
                    ...
                    // if (moment(v.valueDate).isBefore(moment(new Date()).format('YYYYMMDD')) === true) {
                    //     this.xn.msgBox.open(false, '您选择的是已过期的时间,请重新选择');
                    //     return;
                    // }
            ...
            XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                if (v === null) {
                    ...
                    this.xn[this.apiProxyEnum[this.defaultValue]].post(paramBtnOperate.post_url,
                        { mainFlowIds: this.selectedItems.map(x => x.mainFlowId), loanDate: this.loanDate, type: 1 })
                        .subscribe(con => {
            ...
            this.xn[this.apiProxyEnum[this.defaultValue]].post(paramBtnOperate.post_url,
                { mainFlowIds: this.selectedItems.map(x => x.mainFlowId), type: 0 })
                .subscribe(con => {
                    this.initData('D', true);
                });
            ...
            this.xn.router.navigate([`/console/record/avenger/new/sub_approval_again_530`],
                {
                    queryParams: {
                        ...
                        relate: 'mainIds',
                        relateValue: this.selectedItems.map(x => x.mainFlowId),
                    }
                });
            ...
            this.xn.avenger.download('/aprloan/approval/accountDownload',
                { mainFlowIds: this.selectedItems.map(x => x.mainFlowId), 'isProxy': this.listInfo.map(x => x.isProxy)[0] })
                .subscribe((x: any) => {
            ...
            let onemainFlowIds = mainFlowIds[0].split('_');
            ...
            XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                ...
                if (v.cashierDown === '2') {
                    this.xn.avenger.download('/aprloan/approval/loadFinancingWf',
                        {
                            mainFlowIds: this.selectedItems.map(x => x.mainFlowId),
   ...
   */
    private batchSign(btn) {
        ...
        // TODO
        if (mainFlowIds[0].endsWith('lg')) {
            let isOk = false;
            let alert = '';
            const certificateStatus = this.selectedItems.map(x => ({ isPerformance: x.isPerformance, mainFlowId: x.mainFlowId }));
            certificateStatus.forEach(x => {
                if (x.isPerformance === 0 || x.isPerformance === 1) {
                    alert += x.mainFlowId + '、';
                    isOk = true;
                }
            });
            if (isOk) {
                this.xn.msgBox.open(false, `此${alert}的履约证明文件未盖章，保理商无法盖章合同`);
                return;
            }
        }


        if (mainFlowIds[0].endsWith('lg') || mainFlowIds[0].endsWith('wk')) {
            proxyType = 'avenger';
            this.modal = DragonFinancingContractModalComponent;
        } else if (mainFlowIds[0].endsWith('cg')) {
            proxyType = 'avenger';
            this.modal = AvengerFinancingContractModalComponent;
        } else {
            proxyType = 'api';
            this.modal = SignContractsModalComponent;
        }

        this.xn[proxyType].post(btn.post_url, { mainFlowIds: mainFlowIds })
            .subscribe(con => {
                ...
                cons.forEach(x => {
                    if (x.label.includes('国内无追索权商业保理合同')) {
                        ...
                        x['config']['text'] = '乙方（受让方）';
                        // x['config']['position'] = "0,500,50,4#0";
                    } else if (x.label.includes('国内商业保理合同(三方协议)')) {
                ...
                // let contractHide = this.filterContracts(cons);
                //if (contractHide.length !== 0) {
    ...
    finishSignContract() {
        ...
        let mainFlowIds = this.selectedItems.map(x => x.mainFlowId);
        // let isCanLoan = this.selectedItems.map(x => x.isCanLoan);
        // let msgs = [];
        ...
        clearInterval(this.timedpush);
        this.pushApproval();
        this.xn.avenger.post('/jd/jd_finish_sign', { mainFlowIds: mainFlowIds }).subscribe(x => {
            if (x.ret === 0) {
    ...
    private buildParams(addparams: number) {
        ...
        if (addparams !== undefined) {
            if (this.tabConfig.title === '回款管理' && this.subDefaultValue === 'DOING') {
                params = {
            ...
            } else if (this.tabConfig.title === '回款管理' && this.subDefaultValue === 'TODO') {
                params = {
                    start: (this.paging - 1) * this.pageConfig.pageSize,
                    length: this.pageConfig.pageSize,
                    startTime: this.beginTime,
            ...
            for (const search of this.searches) {
                if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
                    if (search.checkerId === 'cnDownTimes' || search.checkerId === 'kjDownTimes') {
                        ...
                        if (this.arrObjs[search.checkerId] === '万科') {
                            params['headquarters'] = '万科企业股份有限公司';
                        } else {
