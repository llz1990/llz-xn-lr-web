 ...
 *  金地模式abs 3.0
 */
export default class Gemdale {
    static readonly flowId = 'financing_pre14';
    static readonly showName = '金地模式';
    ...
    static readonly webUrlBase = `/console/record/record-gemdale/${Gemdale.flowId}}`;
        ...
        headButtons: [
            {
                ...
                click: (xn: XnService, base: CommBase) => {
                    xn.router.navigate([`/console/record/new/${Gemdale.flowId}`]);
                }
            }
        ],
        ...
        rowButtons: [
            {
                ...
                click: (base: CommBase, record, xn, vcr) => {
                    if ((record.status !== 1 && record.status !== 0) || xn.user.roles.indexOf(record.nowRoleId) < 0) {
                        xn.router.navigate([`/console/record/${Gemdale.flowId}/view/${record.recordId}`]);
                    } else {
                        xn.router.navigate([`/console/record/${Gemdale.flowId}/edit/${record.recordId}`]);
                    }
                }
            },
            {
                ...
                click: (base: CommBase, record, xn, vcr) => {
                    xn.router.navigate(['console/record/new'],
                        {
                            queryParams: {
                                'id': 'assignor_information_registration14',
            ...
            },
            {
                ...
                click: (base: CommBase, record, xn, vcr) => {
                    xn.router.navigate(['console/record/new'],
                        {
                            queryParams:
                                { 'id': 'entry_registration_code14', 'relate': 'relatedRecordId', 'relateValue': record.recordId }
                        });
                }
            },
            {
