...
import { BankCardAddComponent } from '../../../../modules/dragon/share/modal/bank-card-add.component';

@Component({
    templateUrl: `./avenger-chose-accountinfo.modal.html`,
    selector: 'avenger-chose-accountinfo',
    styles: [`
    .modal-title{
        height:50px;
    }
...
export class AvengerChoseAccountinfoComponent implements OnInit {
    ...
    public singleChecked(paramItem, index) {
        ...
        if (paramItem['checked'] && paramItem['checked'] === true) {
            ...
            this.selectedItems = this.selectedItems.filter(x => x.cardCode !== paramItem.cardCode);
        } else {
    ...
    addCard() {
        this.modal.close();
        XnModalUtils.openInViewContainer(this.xn, this.vcr, BankCardAddComponent, {})
            .subscribe(v => {
                if (v === null) {
                    return;
                } else {
