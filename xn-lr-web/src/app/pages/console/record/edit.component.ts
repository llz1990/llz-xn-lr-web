...
import { VankeAuditStandardModalComponent } from '../../../public/modal/audit-standard-modal.component';
...
export class EditComponent implements AfterViewInit, AfterViewChecked {
    ...
    public showAuditStandard() {
        let contractObj = [];
        console.log("contractFile", this.localStorageService.caCheMap.get('contractFile'));
        //发票
        let invCheckers = [];
        let step = '';
    ...
    private doSubmit() {
        ...
        if (this.svrConfig.flow.flowId === 'financing6' && 'payables' in this.mainForm.value) {
            // financing6 - 应收账款金额
            ...
            if (['financing_download', 'financing_download14'].includes(this.svrConfig.flow.flowId)) {
                ...
                this.xn.api.download(loadApi[this.svrConfig.flow.flowId], params)
                    .subscribe((v: any) => {
                        ...
                        this.xn.api.post(loadApi[`${this.svrConfig.flow.flowId}_times`], { mainFlowIds: mainFlowIds })
                            .subscribe(data => {
                                this._afterSubmit(
