 ...
 *  定向收款模式-供应商发起签约申请
 */
export class Financing11Flow implements IFlowCustom {
            ...
            .map(json => {
                ...
                contracts.forEach(x => {
                    if (x.label === '国内有追索权商业保理合同') {
                        x['config']['text'] = '甲方（债权人）数字签名';
                    } else if (x.label === '链融科技供应链服务平台共同守则' || x.label === '安心账户（应收账款资金）托管协议（三方）' || x.label === '委托开户通知书') {
