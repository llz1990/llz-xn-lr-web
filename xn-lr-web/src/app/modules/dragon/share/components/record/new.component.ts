...
export class NewComponent implements OnInit, AfterViewInit {
    ...
    private doShow() {
        ...
        if (this.flowId === 'vanke_financing_pre') {
        ...
        if (this.Relate.flowId === 'sub_change_start' || this.Relate.flowId === 'sub_change_capital'
            || this.Relate.flowId === 'sub_person_match_qrs'
            || this.Relate.flowId === 'sub_system_match_qrs'
            || this.Relate.flowId === 'sub_replace_qrs') {
    ...
    private request(method: string) {
        ...
        this.xn.api.dragon.post(`/flow/${method}`, params).subscribe(json => {
            ...
            this.xn.router.navigate([`/dragon/record/record/${this.flowId}`]);
        });
    }
