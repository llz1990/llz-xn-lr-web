...
import { DragonInvoiceDataViewModalComponent } from '../../share/modal/invoice-data-view-modal.component';
...
export class DragonFactorSignComponent extends CommonPage implements OnInit {
    ...
    // 签署保理商合同
    signContract() {
        this.xn.dragon.post('/factoring_sign/signContracts', { mainFlowIds: this.selectedItems.map(x => x.mainFlowId) }).subscribe(con => {
            const cons = con.data;
            ...
            cons.forEach(x => {
                if (x.label.includes('应收账款转让协议之补充')) {
            ...
            XnModalUtils.openInViewContainer(this.xn, this.vcr, DragonFinancingContractModalComponent, cons).subscribe(x => {
                if (x === 'ok') {
                    ...
                    this.xn.dragon.post('/factoring_sign/saveContracts',
                        { contracts: con.data, recordIds: this.selectedItems.map(x => x.recordId) })
                        .subscribe((temp: any) => {
                            if (temp.ret === 0) {
                                this.selectedItems = [];
                                this.onPage({ page: this.paging, pageSize: this.pageSize });
