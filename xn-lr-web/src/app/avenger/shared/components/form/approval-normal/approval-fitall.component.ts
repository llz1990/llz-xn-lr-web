...
import { DragonChoseAccountinfoComponent } from '../../../../../modules/dragon/share/modal/dragon-chose-accountinfo.modal';
...
export class AvengerapprovalComponent implements OnInit {
    ...
    public ngOnInit() {
        ...
        } else if (this.row.type === 'bank-single1' || this.row.type === 'fee-info'
            || this.row.type === 'deal-info' || this.row.type === 'text-info') {
            this.datavalue = !!this.row.value ? JSON.parse(this.row.value) : '';
        } else if (this.row.type === 'compare-info') {
            this.datavalue = !!this.row.value ? JSON.parse(this.row.value) : '';
        }
        this.fromValue();
