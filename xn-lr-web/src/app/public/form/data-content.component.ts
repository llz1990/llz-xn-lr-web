...
export class DataContentComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if(!!this.ctrl_headType.value){
            this.enterprise = this.ctrl_headType.value.includes('{') ? JSON.parse(this.ctrl_headType.value).enterprise : this.ctrl_headType.value;
        ...
        this.uploadCtr = (this.enterprise === '雅居乐地产控股有限公司') || (this.enterprise === '万科' && this.type !== '');
        // 设置可上传的格式
    ...
    downloadTp() {
        ...
        if (!this.uploadCtr) {
            this.alert = '请选择总部公司或者万科类型';
            this.xn.msgBox.open(false, '请选择总部公司或者万科类型');
            return;
        }
