...
import { Component, OnInit, AfterViewInit, ViewContainerRef, Output, EventEmitter, ChangeDetectorRef } from '@angular/core';
import { Params, ActivatedRoute } from '@angular/router';
import { XnService } from '../../../services/xn.service';
import { FormGroup } from '@angular/forms';
import { XnFormUtils } from '../../../common/xn-form-utils';
import { XnUtils } from '../../../common/xn-utils';
import { FlowCustom, IFlowCustom } from './flow.custom';
...
export class NewComponent implements OnInit, AfterViewInit {
    ...
    public authorizationFile() {
        ...
        this.xn.api.download('/custom/vanke_v5/project/download_qrh', {}).subscribe((v: any) => {
            this.xn.loading.close();
            this.xn.api.save(v._body, '确认函-项目公司.zip');
        });
    }
    ...
    private preShow() {
        this.flowCustom.preShow().subscribe((v: any) => {
            this.handleAction(v, () => {
                this.doShow();
            });
        });
    ...
    private doShow() {
        ...
        this.Relate.flowId !== '' ? postObj[this.Relate.relate] = this.Relate.relateValue : postObj.toString();

        if (this.invoiceReplaceParams) {
            Object.assign(postObj, this.invoiceReplaceParams);
        }
        this.xn.api.post('/record/record?method=show_new', postObj).subscribe(json => {
    ...
    private onRelatedRecordIdSelected(obj) {
        ...
        // 去请求该recordId的具体字段
        this.xn.api.post('/record/get_related', {
            flowId: this.flowId,
    ...
    private request(method: string) {
        ...
        if (this.Relate.flowId !== '' &&
            ['financing_download', 'financing_download14', 'pay_order_upload'].indexOf(this.Relate.flowId) === -1) {
            params[this.Relate.relate] = this.Relate.relateValue;
        } else {
            params.toString();
        }
        ...
        this.xn.api.post(`/record/record?method=${method}`, params).subscribe(json => {
            this.presubmitSpecial(params, json);
            this.xn.router.navigate([`/console/record/record/${this.flowId}`]);
        });
    }
    ...
    private presubmitSpecial(params, x) {
        if(this.svrConfig.flow.flowId === 'supplier_upload_information'){
            if (x.ret === 0) {
