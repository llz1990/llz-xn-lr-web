...
@Component({
    templateUrl: './supervisor-edit-modal.component.html',
    styles: [
            `.flex-row {
            display: flex;
            margin-bottom: -15px;
...
export class SupervisorEditModalComponent implements OnInit {
    ...
    open(params: any): Observable<any> {
        ...
        if (this.fileType === 'img') {
            ...
            // 当前此渲染是把iframe构造出来，如果此时直接取iframe是获取不到的，所以用setTimeout在下一次取
            setTimeout(() => {
    ...
    toValue() {
        if (!this.amount || this.amount === '') {
            ...
            this.params['amount'] = this.amount.replace(/\s/g, '').replace(/,/g, '').replace(/，/g, '');
        }
    }
