...
export class StorageRackSelectInputComponent implements OnInit {
    ...
    private selectOptions = SelectOptions.get('storageRack'); // 储架
    constructor(
    ...
    ngOnInit() {
        ...
        console.info('value', this.ctrl1.value);
        if (!!this.ctrl1) {
            this.ctrl1.valueChanges.subscribe(x => {
                if (x !== '') {
                    if (this.row.show !== undefined) {
                        this.xn.dragon.post('/project_manage/old_pool/get_project_list', { headquarters: this.ctrl1.value }).subscribe(x => {
                            this.selectItems = [];
                            x.data.forEach(z =>
    ...
    onBlur() {
        if (this.row.show !== undefined) {
            let choseProjectInfo = this.selectItems.filter(x => x.value === this.ctrl.value);
