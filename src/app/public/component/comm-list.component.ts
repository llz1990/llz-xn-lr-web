 ...
 */
@Component({
    selector: 'xn-comm-list-component',
...
export class CommListComponent extends CommonPage implements OnInit {
    ...
    public bankCardAdd() {
        XnModalUtils.openInViewContainer(this.xn, this.vcr, BankCardAddComponent, {}).subscribe(json => {
            console.log('addModal return', json);
            if (json == 'ok') {
    ...
    public bankCardEdit(row) {
        let bankId = row.bankCode;
        this.xn.api.post('/bank_info_list/query', {
            bankId: bankId
        }).subscribe(x => {
            if (x.ret == 0) {
                let data = x.data.bankInfo;
                ...
                XnModalUtils.openInViewContainer(this.xn, this.vcr, BankCardEditComponent, row).subscribe(json => {
                    console.log('editModal return', json);
                    if (json == 'ok') {
                        this.xn.api.post('/bank_card?method=get', {
                            start: (this.paging - 1) * this.pageSize,
                            length: this.pageSize
    ...
    downAll() {
        ...
        forkJoin(flowIds$).subscribe(async (x: any) => {
            ...
            for (let j = 0; j < arr.length; j++) {
                let row = arr[j];
                for (let rowlog of row.data.logs) {
                    try {
        ...
        if (this.searches.length > 0) {
            ...
            for (const search of this.searches) {
                if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
                    if ((search.type && search.type === 'select') || (search.type === 'listingg')) {
  ...
  */
    ...
    private async rexDownload(params, manflowId) {
        ...
        if (files.length) {
            ...
            let filename = params.enterpriseOrgName + '-' + manflowId + '.zip';
