...
import { CapitalPoolGeneratingContractStampModalComponent } from '../../../share/modal/capital-pool-generating-contract-stamp-modal.component';
import { ContractCreateType, SelectRange } from '../emus';
import { DragonFinancingContractModalComponent } from '../../../share/modal/dragon-asign-contract.modal';
import { CapitalPoolBulkUploadModalComponent } from '../../../share/modal/capital-pool-bulk-upload-modal.component';
...
import { CapitalPoolReturnBackModalComponent } from '../../../share/modal/capital-pool-return-back-modal.component';
import { FileViewModalComponent } from '../../../share/modal/file-view-modal.component';
...
import { CapitalChangeProcessModalComponent } from '../../../share/modal/capital-pool-change-process-modal.component';
...
export class CapitalPoolCommListComponent extends CommonPage implements OnInit {
    ...
    private defaultHeadquarter = '深圳市龙光控股有限公司';
    showOrhide: string = '隐藏';
        ...
        this.route.queryParams
            .pipe(
                map(x => {
                    ...
                    this.fromProject = this.formCapitalPool.fromProject ? this.formCapitalPool.fromProject : false;
    ...
    public changeProcess(item: any) {
        let tradeStatus = this.selectedItems.map(x => ({ tradeStatus: x.tradeStatus, mainFlowId: x.mainFlowId }));
        // let commonStatus = _.difference([101, 102, 103, 104, 99], tradeStatus);
        // if (commonStatus.length !== 4 && commonStatus.includes(104)) {

        // }
        let commonMainFlowId = [];
        let isOk = true;
        ...
        if (!isOk) {
            ...
            this.xn.msgBox.open(false, `交易${alert}还没有完成供应商签署合同，不能发起变更发行流程`);
            return;
        }
    ...
    public exportCapital() {
        ...
        XnModalUtils.openInViewContainer(
            ...
            this.xn.api.dragon.download('/pool/list_excel', param).subscribe((v: any) => {
    ...
    public viewReturnBack(row: any) {
        if (row.returnReason && row.returnReason) {
            XnModalUtils.openInViewContainer(this.xn, this.vcr, CapitalPoolReturnBackModalComponent, row.returnReason).subscribe();
        } else {
    ...
    private buildCondition(searches) {
        ...
        this.mainForm.valueChanges.subscribe(v => {
            ...
            if (changeId !== '' && this.nowTimeCheckId) {
                ...
                if (beginTime === this.createBeginTime && endTime === this.createEndTime) {
                    // return;
                } else {
    ...
    private buildParams() {
        ...
        // 搜索处理
        let a = this.searches.filter(x => !XnUtils.isEmpty(this.arr[x.checkerId]));
        this.searches
    ...
    private doGenerateOrSign(url, params) {
        this.xn.api.dragon.post(url.generate, params)
            .pipe(
                map(con => {
                    this.xn.loading.close();
                    const contract =
    ...
    private dragonContracts(contractCreateType: ContractCreateType, noStamp = true) {
        return {
            [contractCreateType]: this.dragonContractCreateUrl,
            [`${this.updateContractKey(contractCreateType)}`]: this.dragonContractSaveUrl,
    ...
    private isDifferentCompany() {
        const company = XnUtils.distinctArray(this.rows.filter(x => x.headquarters).map(c => c.headquarters)) || [];

        return company.length > 1;
    }

    private hasSelectRow() {
        const selectedRows = this.rows.filter(x => x.checked && x.checked === true);
