...
import { FlowCustom, IFlowCustom } from '../../../../avenger/flow/flow-custom';
...
import { VankeViewChangeAccountComponent } from '../../../../modules/dragon/share/modal/vanke-change-account.component';
class checkerDatas {
    mainFlowId: string = '';
    recordId: string = '';
    procedureId: string = '';
    ...
    private showSubmit(recordId, fresh?) {  // fresh 为可选，true的时候则刷新，不拉取actions的最后一项，拉checker
        this.xn.avenger.post('/flow/showSubmit', {
            ...
            if (this.svrConfig.record.flowId === 'financing_500' || this.svrConfig.record.flowId === 'financing_501') {
                let stopLength = this.svrConfig.actions.length;
 ...
 */
    public afterSubmitandGettip(svrConfig: any) {
        ...
        if (svrConfig.flow.flowId === 'sub_factoring_change_520' && svrConfig.procedure.procedureId === 'review') {
            this.xn.msgBox.open(false, '提交成功！请供应商在【首页-待办任务】中填写变更信息');

        }
        if (svrConfig.flow.flowId === 'sub_customer_verify_520' && svrConfig.procedure.procedureId === 'operate') {
            this.xn.msgBox.open(false, '提交成功,下一步请客户经理复核人在【首页-待办任务】中完成【复核】的待办任务');

        }
        if (svrConfig.flow.flowId === 'sub_customer_verify_520' && svrConfig.procedure.procedureId === 'review') {
            this.xn.msgBox.open(false, '提交成功！请风险复核人在【首页-待办任务】中完成【风险复核】的待办任务。');

        }
        if (svrConfig.flow.flowId === 'sub_financing_verify_520' && svrConfig.procedure.procedureId === 'operate') {
            this.xn.msgBox.open(false, '提交成功！请高管复核人在【首页-待办任务】中完成【高管复核】的待办任务。');

        }
        if (svrConfig.flow.flowId === 'sub_financing_verify_520' && svrConfig.procedure.procedureId === 'review') {
        ...
        this.xn.avenger.post('/in_flow/reject/showData', { mainFlowId: this.mainFlowId }).subscribe(x => {
            if (x.data && x.data.backCompany && x.data.dealApp && x.data.rate) {
                ...
                XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params).subscribe(v => {
                    ...
                    let datalist = this.updatedata.filter(index => {
                        return index.mainFlowId !== '';
                    });
                    this.xn.avenger.post('/in_flow/reject/updateData', { checkerDatas: datalist }).subscribe(x => {
                        if (x.data) {
