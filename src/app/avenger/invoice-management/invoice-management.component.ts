...
export class DragonInvoiceManagementComponent implements OnInit {
    ...
    public formModule: string = 'avenger-input';
     ...
     * @param paramFile
     */
    public viewFiles(fileId,mainFlowId) {
        let params = {
            file:JSON.stringify([{fileId: fileId+'.pdf', filePath: mainFlowId}]),
            isAvenger:true,
            isInvoice:true
        };
    ...
    public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        if(this.selectedItems && this.selectedItems.length > 0){
            let JudgeCondition = [];
            let msg = "";
            let statusOk = this.selectedItems.every((x) =>{
                return [5,6].includes(x.status);
            });
            if(!statusOk){
                this.xn.msgBox.open(false,`交易状态为已放款、已回款的业务可发起${paramBtnOperate.operate === 'sub_nuonuocs_blue'?'开票':'作废'}申请流程`);
                return ;
            }
            ...
            let isOk = this.selectedItems.every((x) =>{
