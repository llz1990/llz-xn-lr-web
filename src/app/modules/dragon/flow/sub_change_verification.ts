...
export class SubChangeVerification implements IFlowCustom {
    ...
    preSubmit(svrConfig: any, formValue: any): Observable<any> {
        if (svrConfig.procedure.procedureId === 'operate') {
            let contractFile = formValue.pdfProjectFiles;
            let correctFile = formValue.correctFile;
            let alert = '';
            try {
                ...
                if (correctFile === '') {
                    ...
                    // let others = JSON.parse(svrConfig.checkers.filter(x => x.checkerId === 'performanceFile')[0].other);
                    // others.show = true;
                    // svrConfig.checkers.filter(x => x.checkerId === 'performanceFile')[0].other = JSON.stringify(others);
                } else {
                    alert = '';
                }
                if (alert !== '') {
