...
export class BankNewComponent implements OnInit, AfterViewInit {

    formModule = 'bank-input';
    flowId: string;
    Relate = {
        flowId: '',
        relate: '', // 关联流程名
    ...
    ngAfterViewInit() {
        ...
        this.route.queryParams.subscribe(params => {
            ...
            let relateValues = params.relateValue || JSON.parse(sessionStorage.getItem('relate_value'));
            this.Relate = {
                flowId: params.id,
                relate: params.relate, // 关联流程名
                relateValue: this.judgeDataType(relateValues)?relateValues:[relateValues]
            };
    ...
    private doShow() {
        ...
        this.xn.avenger.post('/flow/showNew', postObj).subscribe(json => {
            ...
            this.flowCustom.postGetSvrConfig(json.data);
            this.svrConfig = XnFlowUtils.handleSvrConfig(json.data);
            this.buildRows();
        });
    }
    ...
    private request(method: string) {
        ...
        this.xn.api.avenger.post(`/flow/${method}`, params).subscribe(json => {
            this.presubmitSpecial(params, json);
            this.xn.router.navigate([`/console/bank-puhuitong/record/record/${this.flowId}`]);
        });
    }
    ...
    private presubmitSpecial(params, x) {
        if(this.svrConfig.flow.flowId === 'sub_bank_push_supplier_sign'){
            if (x.ret === 0) {
                this.xn.msgBox.open(false, `提交成功！\n 下一步请复核人在【首页-待办任务】中完成【复核】的待办任务。`);
            }
        }else if (this.svrConfig.flow.flowId === 'sub_bank_platform_add_file'
            || this.svrConfig.flow.flowId === 'sub_bank_platform_change_file') {
            if (x.ret === 0) {
