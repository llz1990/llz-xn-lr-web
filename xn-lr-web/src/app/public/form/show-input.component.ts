...
export class ShowInputComponent implements OnInit {
        ...
        if (!isNullOrUndefined(this.memo)) {
            ...
            if (this.row.type === 'radio') {
                ...
                if (this.row.data.includes('}')) {
                    ...
                    let secondSelect = SelectOptions.get('enterprise_dc').filter(x => x.value === JSON.parse(this.row.data).enterprise);
                ...
                for (const array of arrays) {
                    if (array.checked === true) {
                        ...
                        if (array.data && array.data.length) {
                            labels.push(`【 公司性质：${array.data.filter(x => x.checked === true).map(y => y.label).join(',')}】`);
                        }
                    }
                }
                this.label = labels.join(', ');
                // console.log('..................', this.label);
            } else if (this.row.type === 'dselect') {
    ...
    formResize(){
        if(this.fileGroup && this.fileGroup.nativeElement){
            let $parentForm = $(this.fileGroup.nativeElement).closest('.form-group');
            //获取父元素宽度
            let parentWidth = $parentForm.outerWidth();
            let col2Width  =  $parentForm.children('.xn-control-label').outerWidth(true);
            let col8Left = ($parentForm.children('.col-sm-6').outerWidth(true) - $parentForm.children('.col-sm-6').width())/2;
            //设置当前元素样式
    ...
    viewPerformance(item: any) {
        let debtUnits = this.newSvrconfig.checkers.filter(x => x.checkerId === 'debtUnit');
        let paybables = this.newSvrconfig.checkers.filter(x => x.checkerId === 'payables');
        let projectCompanys = this.newSvrconfig.checkers.filter(x => x.checkerId === 'projectCompany');
        let infos = this.newSvrconfig.checkers.filter(x => x.checkerId === 'contractFile');

        let debtUnit = debtUnits && debtUnits.length ? debtUnits[0].data : "";
        let projectCompany = projectCompanys && projectCompanys.length ? projectCompanys[0].data : "";
        let paybable = paybables && paybables.length ? paybables[0] : "";
        let info = infos && infos.length ? infos[0].data : "";
        const checkers = [
            {
                title: '合同名称',
    ...
    public checkBalanceData(paramItems: any, paramIndex: number): boolean {
        ...
        let d = paramItems.data[paramIndex];
    ...
    public checkData(parmaItems: any, paramIndex: number): boolean {
        ...
        let d = parmaItems.data[paramIndex];
        if (d === undefined || d === null) {
            return false;
        }
    ...
    public onViewBankInvoice(item: any) {
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, BankInvoiceViewModalComponent, item).subscribe((v) => {
            ...
            const params = {
                type: 'invoice',
                consistent: this.items.every(x => x.consistent)
            };
    ...
    public onViewFile(item: any) {
        ...
        if (item.businessLicenseFile) {
            let orgName = this.newSvrconfig.checkers.filter(x=>x.checkerId === 'orgName')[0].data;
            let item1= Object.assign({}, item, {orgName});
