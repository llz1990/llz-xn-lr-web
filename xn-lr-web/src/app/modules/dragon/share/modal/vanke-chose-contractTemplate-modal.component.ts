...
export class VankechoseTemplateModalComponent implements OnInit {
  ...
  */
     ...
     */
    clearData(head: any) {
        let clearindex = this.selectedItems.findIndex((item) => { return item.id === head.id });
        head.checked = false;
        this.data.map(item => {
            if(item['id'] === head['id']){
                item['checked'] = false;
     ...
     */
    SureAddComp() {
        let secondTemplateId = this.selectedItems.map(x => x.id);
        this.xn.dragon.post('/contract/second_contract_info/add_second_template',
            { project_manage_id: this.params.project_manage_id, secondTemplateId: secondTemplateId }).subscribe(x => {
                if (x.ret === 0) {
