...
import DragonchangeCapitalTabConfig from '../../../../common/change-capital.config';
...
@Component({
    ...
    <div class="table-head" [ngStyle]="{'padding-right':items.length>10?'17px':'0px'}">
      <table>
      <ng-container *ngFor="let head of currentTab.heads">
      <colgroup>
        <col [span]="currentTab.heads.length+1" [ngStyle]="{'width':head.type==='mainFlowId'?'14%':head.width}" />
        <col />
      </colgroup>
    </ng-container>
...
@DynamicForm({ type: 'dragon_add_list', formModule: 'dragon-show' })
export class DragonchangeCapitalshowComponent implements OnInit {
