...
export class AvengerplatbothContractComponent implements OnInit {
    ...
    */
    ...
    public handleRowClick(paramItem) {
        ...
        // } else {
        //     // let upstreaminfo = this.items.filter(x => x.upstreamContractNum !== undefined);
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, AvengerBothPlatContractModalComponent, params)
            .subscribe(v => {
                ...
                if (v.action === 'cancel') {
                    ...
                    let contractFile1 = curentparam.contractFile;
                    let receicevalueinfo1 = paramItem.receiveType;
                    let reveiveType1 = v.contractType.receiveType === '' ? paramItem.twoselect : v.contractType.receiveType.twoselect;
                    let reveiveType2 = v.contractType.receiveType === '' ? paramItem.threeselect : v.contractType.receiveType.threeselect;
                    ...
                    let index = this.items.findIndex(x => x.id === paramItem.id);
                    if (index >= 0) {
