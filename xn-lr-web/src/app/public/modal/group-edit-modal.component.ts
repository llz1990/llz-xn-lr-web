...
@Component({
    ...
    styles: [
        ...
        `.this-control {display: block; vertical-align: top; font-size: 13px; width: 230px; margin-bottom: 5px; margin-right: 20px;}`,
        ...
        `.xn-control-label { width: 100px; text-align: left; padding-top: 0px; padding-bottom: 3px; font-size: 12px;}`,
        `.xn-control-content { width: 300px; text-align: left; padding-top: 0px; padding-bottom: 3px; font-size: 12px;}`,
...
export class GroupEditModalComponent implements OnInit {
    ...
    open(params: any): Observable<any> {
        ...
        if (params && params.amount && params.amount !== '') {
            ...
            this.factoringDate = params.factoryDate;
            this.num = params.num;
            this.computeDay();
        }

        this.buildPic(params.files);
        this.buildForm();
    ...
    buildPic(files) {
        this.total = files && files.length || 0;
        if (files && files.length > 0) {
            this.fileSrc = `/api/attachment/view?key=${files[0].fileId}`;
        } else {
            this.fileSrc = '';
        }
    }
    ...
    buildForm() {
        this.rows = [
            {
                title: '付款确认书', checkerId: 'files', type: 'mfile',
                options: {
        ...
        this.mainForm.valueChanges.subscribe(v => {
            this.params['files'] = v.files !== '' ? JSON.parse(v.files) : '';
            this.buildPic(this.params['files']);
        });
    }
    ...
    isValid() {
        return !!(this.amount === '') || !this.factoringDateTemp || !!(this.num === '') || !!(this.params['files'] === '');
    }
    ...
    onOk() {
        ...
        console.log('close-params: ', this.params);
