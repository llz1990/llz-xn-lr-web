...
export class MachineNewComponent implements OnInit, AfterViewInit {
    ...
    private doShow() {
        ...
        if (this.Relate.flowId === 'sub_factoring_retreat') {
        ...
        this.xn.dragon.post('/flow/showNew', postObj).subscribe(json => {
    ...
    private buildRows(): void {
       ...
       // console.info('flowId==>', this.flowId);
        this.rows = this.rows.concat({
            name: 'memo',
            required: false,
            type: 'textarea',
            validators: this.flowId === 'sub_factoring_retreat' ? {
                maxlength: 500,
            } : {},
    ...
    private request(method: string) {
        ...
        this.xn.api.dragon.post(`/flow/${method}`, params).subscribe(json => {
            this.xn.router.navigate([`/machine-account/record/record/${this.flowId}`]);
        });
    }
