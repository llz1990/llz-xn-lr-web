...
export class ParamInputModel {
  ...
  public deBuit?: string = '';
  /** 应收账款金额*/
  public receive?: string = '';
  /** 原资产池名称 */
  public capitalPoolName?: string = '';
}
@Component({
  template: `
  <modal #modal [backdrop]="'static'" [keyboard]="false" [animation]="false">
    <form class="form-horizontal" (ngSubmit)="handleSubmit()">
      ...
      <table class="table table-bordered table-hover file-row-table text-center" width="100%" >
        <thead>
          <tr>
            <td><span class="span-line">序号</span></td>
            <td><span class="span-line">交易id</span></td>
            <td><span class="span-line">收款单位</span></td>
            <td><span class="span-line">应收账款金额</span></td>
            <td><span class="span-line">原资产池</span></td>
          </tr>
        </thead>
        <tbody>
          <tr *ngFor="let sub of selectedItems;let i=index">
            <td>{{i+1}}</td>
            <td><a [innerHTML]="sub.mainFlowId" href="javaScript:void (0)"
              (click)="viewProcess(sub.mainFlowId)"></a>
            </td>
            <td>{{sub.debtUnit}}</td>
            <td>{{sub.receive}}</td>
            <td>{{params.capitalPoolName}}</td>
          </tr>
        </tbody>
      </table>
      </div>
      ...
      <modal-footer>
        ...
        <button type="submit" class="btn btn-success" (click)='enterCapital()'>{{paramButton}}</button>
      </modal-footer>
    </form>
  </modal>
  `
})
export class VankeDeleteTransactionEditModalComponent {
  ...
  open(params): Observable<any> {
    ...
    this.paramType = params.type === 1 ? `是否要将以下交易移入资产池：${params.capitalName}资产池 ？（共${this.selectedItems.length}笔）` :
      `是否要移除以下交易？（共${this.selectedItems.length}笔）`;
  ...
  enterCapital() {
    const param = {
      mainFlowIdList: this.selectedItems.map(x => x.mainFlowId),
      capitalPoolId: this.capitalPoolId
    };
    this.xn.dragon.post(this.params.post_url, param).subscribe(x => {
      if (x.ret === 0) {
        this.xn.msgBox.open(false, `${this.paramButton}资产池成功`);
        this.close(null);
      }
    });
  }
