 ...
 * All rights reserved.
 *
 * @file：flow-detail.component.ts
 * @summary：交易详情
 * @version: 1.0
 * **********************************************************************
 * revision             author                    date
 * 1.0                 wangqing               2019-09-17
 * reason
 * 删除多余代码
 * **********************************************************************
 */
...
import { XnAsset } from '../../../../../common/xn-asset';
...
export class BankFlowDetailComponent implements OnInit {
    ...
    ngOnInit() {
        this.route.params.subscribe((params: Params) => {
            ...
            if (params['proxy'] === undefined) {
                this.xn.avenger.post('/flow/main/detail', {
                    ...
                    this.showData = this.params.numData && this.params.numData.length > 0; // 显示数据仓
                    this.numData = this.params.numData;
                });
            } else {
                // this.currentStatus = params['currentStatus'];
                this.xn.avenger.post('/list/main/detail', {
                    mainFlowId: this.mainFlowId
                }).subscribe(json => {
                    this.params = json.data;
                    this.params.supplierOrgName = this.params.supplierName;
                    this.params.factoringOrgName = this.params.factoringName;
                    this.params.upstreamName = this.params.upstreamName;
                    // 先转换对象
                    this.showTab = this.params.assets && this.params.assets.length > 0; // 显示数字资产
