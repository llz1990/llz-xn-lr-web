...
export class PaymentNoticeIndexComponent implements OnInit {
    ...
    public downloadNotices() {
        const data = [];
        const mapNotice = this.selects.map(x => x.noticeContracts);
    ...
    public calcTd(): boolean {
        return this.data.find(x => x.noticeStatus === '待下载');
    ...
    private calcStatus() {
        ...
        const find = this.data.find(x => x.checked === undefined || x.checked === false);
        if (!find) {
