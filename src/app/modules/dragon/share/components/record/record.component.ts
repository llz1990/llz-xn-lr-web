...
export class DragonRecordComponent implements OnInit {
    ...
    editAction(val: any) {
        ...
        if ((data.status !== 1 && data.status !== 0) || !XnUtils.getRoleExist(data.nowRoleId, this.xn.user.roles, 1)) {
            this.xn.router.navigate([`/dragon/record/${this.flowId}/view/${data.recordId}`]);
        } else {
            this.xn.router.navigate([`/dragon/record/${this.flowId}/edit/${data.recordId}`]);
        }
    }

    onNewRecord() {
        this.xn.router.navigate([`/dragon/record/new/${this.flowId}`]);
    }
}
