 ...
 */
@Component({
    selector: 'app-contract-control-index-component',
    templateUrl: './contract-control-index.component.html',
    styles: [`
        .content-p {
            padding: 10px;
        }
...
export class ContractControlIndexComponent implements OnInit {
    ...
    public onPage(page: number) {
        ...
        this.xn.api.post(`/yb/risk1/contract_control/get_data`, params).subscribe(v => {
    ...
    viewDetail(item, e: MouseEvent) {
        this.xn.router.navigate([`/console/transaction-control/contract/enterprise`], {queryParams: {org: item.orgName}});
        // 禁止a标签的默认行为
