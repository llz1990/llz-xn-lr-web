    ...
    private initDate(beginTime, endTime) {
        const afterSelect = ((start, end) => {
            if (Number(start) !== -28800000) { // 不选择时间
                ...
                $(this.configDate.nativeElement).val('选择时间段');
                this.toValue(undefined);
            }
        }).bind(this);

        $(this.configDate.nativeElement).daterangepicker({
            startDate: beginTime && moment.unix(beginTime / 1000) || 0, // 初始化时间兼容后退时间
            endDate: endTime && moment.unix(endTime / 1000) || 0,
            opens: 'right',
            drops: this.position,
