...
import { MachineInvoiceListComponent } from '../../../machine-account/share/modal/invoice-list-modal.component';
...
export class DragonTransactionsListComponent implements OnInit {
    ...
    public supplementTransInfo() {
        ...
        this.yjlSelectItems = this.selectedItems.filter(x => String(x.isProxy) === '6' && x.headquarters==="雅居乐地产控股有限公司"); // 雅居乐业务
        this.notYjlSelectItems = this.selectedItems.filter(x => String(x.isProxy) !== '6' || x.headquarters !== "雅居乐地产控股有限公司"); // 非雅居乐业务
        if(this.yjlSelectItems.length>0){
            if (this.yjlSelectItems.some(x => (!x.factoringEndDate || x.factoringEndDate === '')
     ...
     * @param i 下标
     */
    public handleRowClick(item, paramBtnOperate: ButtonConfigModel, i: number) {
        // let mainFlowId = this.selectedItems.find(x => x.mainFlowId);
        paramBtnOperate.click(this.base, item, this.xn, this.hwModeService);
    }

    //查看发票详情
    handlerowInvoiceClick(item, paramBtnOperate: ButtonConfigModel, i: number) {
