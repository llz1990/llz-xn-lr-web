...
export class XNCurrency {
     ...
     */
    public constructor(paramValue: number | string | XNCurrency = 0) {
        if (XNCurrency.isCurrency(paramValue)) {

            this.m_IntValue = (paramValue as XNCurrency).m_IntValue;
            this.m_ErrFlag  = (paramValue as XNCurrency).m_ErrFlag;
            this.m_ErrMsg   = (paramValue as XNCurrency).m_ErrMsg;
        } else {
            let v = Parse(paramValue as number | string);
            if (v.errFlag) {
                this.m_IntValue = 0;
                this.m_ErrFlag  = true;
    ...
    /** 判断指定的对象，是不是指定的XNCurrency对象 */
    public static isCurrency(paramV: any) {
        return paramV instanceof XNCurrency;
    }
    ...
    /**
     * 赋值
     * @param {number | string | XNCurrency} paramValue 新值
     * @return {XNCurrency} 返回当前对象
     */
    public assign(paramValue: number | string | XNCurrency = 0): XNCurrency {
        if (XNCurrency.isCurrency(paramValue)) {
            this.m_IntValue = (paramValue as XNCurrency).m_IntValue;
            this.m_ErrFlag  = (paramValue as XNCurrency).m_ErrFlag;
            this.m_ErrMsg   = (paramValue as XNCurrency).m_ErrMsg;
    ...
    /**
     * 加一个值
     * @param {number | string | XNCurrency} paramNumber 值
     * @return {XNCurrency} 返回计算后的Currency对象
     */
    public add(paramNumber: number | string | XNCurrency): XNCurrency {
        let r = new XNCurrency(paramNumber);
     ...
     * 自加一个值
     * @param {number | string | XNCurrency} paramNumber 值
     * @return {XNCurrency} 返回自己
     */
    public selfAdd(paramNumber: number | string | XNCurrency): XNCurrency {
        let r = new XNCurrency(paramNumber);
        if (!r.isErr && !this.isErr) {
     ...
     * @return {XNCurrency} 返回计算后的Currency对象
     */
    public sub(paramNumber: number | string | XNCurrency): XNCurrency {
        let r = new XNCurrency(paramNumber);
        if (!r.isErr && !this.isErr) {
            r.m_IntValue = this.m_IntValue - r.m_IntValue;
        }
     ...
     * @return {XNCurrency} 返回计算后的Currency对象
     */
    public selfSub(paramNumber: number | string | XNCurrency): XNCurrency {
        let r = new XNCurrency(paramNumber);
        if (!r.isErr && !this.isErr) {
            this.m_IntValue = this.m_IntValue - r.m_IntValue;
        }
     ...
     * @return {XNCurrency} 返回计算后的Currency对象
     */
    public mul(paramNumber: number | string): XNCurrency {
        let r = new XNCurrency();
    ...
    public selfMul(paramNumber: number | string): boolean {
        ...
        let r = new XNCurrency(v.value * this.value);
        if (r.isErr) {
            return false;
        } else {
            this.assign(r);
     ...
     * @return {XNCurrency} 返回计算后的Currency对象
     */
    public div(paramNumber: number | string): XNCurrency {
        let r = new XNCurrency();
        let v = __toNumber(paramNumber);
     ...
     *  - false 表示计算失败
     */
    public selfDiv(paramNumber: number | string): boolean {
        let v = __toNumber(paramNumber);
        if (!v.result) {
            return false;
        }
        ...
        let r = new XNCurrency(this.value / v.value);
        if (r.isErr) {
            return false;
        } else {
            this.assign(r);
     ...
     */
    public Chinese(paramOpts: {prefix?: string, negative?: string, zheng?: string} = null): string {
        let stPrefix = '人民币';
        let stNegative = '负';
        let stZheng = '整';
        if (paramOpts !== null && paramOpts !== undefined && _.isObject(paramOpts)) {
            // 人民币前缀
            if (_.isString(paramOpts.prefix)) {
                stPrefix = paramOpts.prefix;
            }
            // 负数前缀
            if (_.isString(paramOpts.negative)) {
                stNegative = paramOpts.negative;
            }
            // 整或正字
            if (_.isString(paramOpts.zheng)) {
     ...
     */
    public format(paramUseSymbol: boolean = false, paramCNYsplit: boolean = false) {
