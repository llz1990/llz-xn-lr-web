...
export class VankeCapitaladdCeilComponent implements OnInit {
    ...
    changeRece(paramInfo, paramIndex) {
        ...
        // },
        {
            ...
            type: 'date',
            required: 0,
            value: moment(paramInfo.date).format('YYYY-MM-DD'),
        }
        ];
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, EditModalComponent, params)
            .subscribe((v) => {
                if (v === null) {
                    ...
                    this.datalist01.push({ invest_tier_id: 0, investTierName: v.investTierName,
                         tierReceive: this.ReceiveData(v.tierReceive), productendTime: moment(v.productendTime).valueOf() });
                    this.ctrl.setValue(this.datalist01);
