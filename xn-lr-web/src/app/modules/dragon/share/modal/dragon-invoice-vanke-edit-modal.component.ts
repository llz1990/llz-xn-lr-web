 ...
 */
@Component({
    templateUrl: './dragon-invoice-vanke-edit-modal.component.html',
    styleUrls: [`./dragon-invoice-view-modal.component.css`],
    providers: [
        PdfViewService
    ]
})
export class DragonInvoiceVankeEditModalComponent implements OnInit {
    ...
    open(params: any): Observable<any> {
        ...
        // 发票地址
        this.fileSrc = params.fileId !== '' ? this.xn.file.dragonView(params) : '';
    ...
    onOk() {
        ...
        // 检查重复发票号码
        const repeatChecked = this.repeatCheck(this.fphm.val(), this.fpdm.val(), this.params);
        if (!repeatChecked) {
            return;
        }
        // 判断是金额或校验码
        ...
        if (!retVerify) {
            ...
            this.xn.dragon.postMap('/file/invoice_check', params).subscribe(json => {
                ...
                if (json.ret === 0) {
                    if (json.data.type === 'single') {  // 老发票验证
                        ...
                        this.xn.msgBox.open(false, '发票验证成功', () => {
                            this.xn.dragon.post('/file/invoice_status', { invoiceNum: params.invoiceNum, invoiceCode: params.invoiceCode }).subscribe(x => {
                                this.close({
                                    action: 'ok',
                        ...
                        this.xn.msgBox.open(false, '发票验证成功', () => {
                            this.close({
                                ...
                                amount: json.data.data.amount || 0,
    ...
    onCancel() {
        ...
        this.xn.dragon.post('/file/invoice_status', { invoiceNum: this.fphm.val(), invoiceCode: this.fpdm.val() }).subscribe(x => {
            this.close({
                invoiceNum: this.fphm.val(),
                invoiceCode: this.fpdm.val(),
                invoiceDate: this.kprq.val(),
                invoiceAmount: this.hsje.val(),
                amount: this.kpje.val(),
                status: x.data.status || 1,
                action: 'ok',
    ...
    onTest() {
        ...
        this.xn.dragon.post('/file/invoice_status', { invoiceNum: this.fphm.val(), invoiceCode: this.fpdm.val() })
            .subscribe(x => {
                this.close({
                    action: 'ok',
                    status: x.data.status || 1,
    ...
    // 检查发票代码
    repeatCheck(fphm, fpdm, params): boolean {

        for (const temp of params.temps) {
            if (Number(temp.invoiceNum) === Number(fphm) && Number(temp.invoiceCode) === Number(fpdm)) {
