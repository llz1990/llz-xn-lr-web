...
@DynamicForm({ type: 'person-list', formModule: 'dragon-show' })
export class DragonPersonListShowComponent implements OnInit,AfterViewInit,OnDestroy {
    ...
    ngOnInit() {
        if(this.row.flowId === 'sub_system_match_qrs'){
            let fileUploadCheckers = this.svrConfig.actions.find((action)=>{
            ...
            let headquarters = this.svrConfig.actions[0].checkers.find(h=>h.checkerId === 'headquarters');
