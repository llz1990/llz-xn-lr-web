        ...
        this.examData.forEach(exam => {
            ...
            if (exam.idLeft === 'payTime' && exam.idRight === 'contractValidTime'
                ...
                if (exam.valueLeft.length === 1 && exam.valueRight.length === 1) {
                    ...
                    exam.display = true;
                    // 如果所有元素都相等怎村子-左右两边想等
                    const find = exam.valueRight.find(x => x !== exam.valueRight[0]);
                    // 找出右边最小的值
                    ...
                    exam.display = true;
                    const find = exam.valueLeft.find(x => x !== exam.valueLeft[0]);
                    // 找出左边最大的值
                    ...
                    const findRight = exam.valueRight.find(x => x !== exam.valueRight[0]);
                    const findLeft = exam.valueLeft.find(x => x !== exam.valueLeft[0]);
            ...
            if (exam.idLeft === 'enterprise' && exam.idRight === 'contractBuySale'
                ...
                if (value.length) {
                    const find = value.find(x => x === exam.valueLeft.toString());
                    if (find) {
                        exam.valueCenter = '等于';
                        exam.color = 'pass-color';
                    } else {
            ...
            if (exam.idLeft === 'inoviceGiver' && exam.idRight === 'contractSaler'
                ...
                if (value.length) {
                    const find = value.find(x => x !== exam.valueRight.toString());
                    if (find) {
                        exam.valueCenter = '不等于';
                        exam.color = 'fail-color';
                    } else {
