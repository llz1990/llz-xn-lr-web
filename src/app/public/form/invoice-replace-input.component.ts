...
export class InvoiceReplaceInputComponent implements OnInit {
    ...
    public onRemove(val: any, i) {
        ...
        this.btnStatus = this.items.some(x => x.clicked === true);
        this.checkedAllStatus = !this.items.some(x => x.clicked === undefined || x.clicked === false) && this.items.length > 0;
        this.toValue();
        this.getDeputeMoney();
    ...
    public handleSelect(index) {
        ...
        this.checkedAllStatus = !this.items.some(x => x.clicked === undefined || x.clicked === false);
    }
