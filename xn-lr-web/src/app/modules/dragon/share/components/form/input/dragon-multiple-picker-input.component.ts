...
export class DragonMultiplePickerInputComponent implements OnInit,AfterViewInit,OnDestroy {
    ...
    selectValue: string=''; //修改流程显示已有值
    selectType: boolean = false;  //通用false/非通用true
    isDisabled: boolean = false; //删除流程不可修改
    subResize: any;
    ...
    onSelect() {
        ...
        let params = Object.assign(obj,{});   //this.config
            ...
            if (v.action === 'cancel') {
                ...
                if(this.row.checkerId !== 'contractTemplate') {
                    ...
                    this.items = this.originalItems.map((c)=>{
                        let templateFile = JSON.parse(c.templateFile);
    ...
    //     this.items = this.items.filter(x => x[label] !== value);
    //     this.setValueByFiles();
