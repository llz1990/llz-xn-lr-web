...
@Component({
    selector: 'xn-fee-bank-discount',
    templateUrl: './fee-bank-discount.component.html',
    styles: [
        `
            .table {
                font-size: 13px;
...
export class FeeBankDiscountComponent implements OnInit {
    @Input() row: any;
    @Input() form: FormGroup;

    theadText = ['核心企业 ID', '核心企业名称', '核心企业类型', '是否可融资', '融资可用额度', '保证金比例(%)', '保理保证期限', '年化利率(%)'];
    ...
    onUploadExcel(e) {
        ...
        // 上传excel
        this.xn.api.upload('/llz/bank_discounting/excel_upload', fd).subscribe(
            res => {
    ...
    private initData() {
        this.xn.api
            .post('/llz/bank_discounting/list', {
                flowId: this.flowId
            })
            .subscribe(v => {
    ...
    private initEditData() {
        this.xn.api
            .post('/llz/bank_discounting/check_data', {
    ...
    private checkChanged(originData, newData = []) {
        ...
        resData.forEach(item => {
            if (data.findIndex(y => item.appId === y.appId) >= 0) {
                ...
                const temp = data.find(x => x.appId === item.appId && x.discountDays === item.discountDays);
                keys.forEach(key => {
