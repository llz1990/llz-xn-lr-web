...
export class AvengerRateInputComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.calcAlertClass();
        if(!!this.ctrl.value){
            let x = this.ctrl.value;
            let tradeListVal = JSON.parse(this.ctrl_tradeList.value);
            tradeListVal.map((listVal)=>{
                if(listVal['changeEnd']!==null && listVal['changeEnd']!==undefined){
                    let unitPrice = Number(listVal['changeEnd'])/(1+Number(x/100));
        ...
        this.ctrl.valueChanges.subscribe((x) => {
            if(this.row.flowId === 'sub_nuonuocs_blue' && this.regExp.test(x)){
                let tradeListVal = JSON.parse(this.ctrl_tradeList.value);
                tradeListVal.map((listVal)=>{
                    if(listVal['changeEnd']!==null && listVal['changeEnd']!==undefined){
                        let unitPrice = Number(listVal['changeEnd'])/(1+Number(x/100));
