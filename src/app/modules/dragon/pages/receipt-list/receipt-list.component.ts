...
import { ReceiptSignReturnModalComponent } from '../../share/modal/receipt-sign-return-modal.component';
...
export class ReceiptListComponent extends CommonPage implements OnInit {
    ...
    public audit(items?) {
        const selectedRows = items
            ? [items]
            : this.rows.filter(
                x => x.checked && x.checked === true && !this.isAudited(x)
            );
...
               }
    ...
    public signContracts() {
        ...
        //     return;
        // }
        const rows = this.rows.filter(x => x.checked && x.checked === true);
    ...
    againSignCons(paramCons: any): void {
        let contract = JSON.parse(paramCons);
