...
export default class DragonCommBase {
    ...
    onList(params) {
        ...
        //         this.onDefaultList(params);
        //     } else if (type === 'B') {
        //         this.onAvengerDefaultList(params);
        //     }
        // }
    }
    // avengeronList(params, type) {
    //     console.info('type', type);
    //     if (this.superConfig.list && this.superConfig.list.onList) {
    //         this.superConfig.list.onList(this, params);
    //     } else {
    ...
    onDefaultList(params) {
        this.xn.dragon.post(this.superConfig.apiUrlBase + '?method=get', params).subscribe(json => {
            this.com.total = json.data.count;
            this.com.rows = json.data.data;
        });
    }
    // 采购融资代办列表接口调试
    onAvengerDefaultList(params) {
        this.xn.avenger.post(this.superConfig.avengerUrlBase, params).subscribe(json => {
            this.com.total = json.data.count;
            this.com.rows = json.data.data;
        });
    ...
    onDefaultDetail(params) {
        this.xn.dragon.post(this.superConfig.apiUrlBase + '?method=detail', {
    ...
    onListDetail(params) {
        this.xn.dragon.post(this.superConfig.apiUrlDetail + '?method=detail', {
            where: this.buildDetailParams(params)
        }).subscribe(json => {
            this.buildRows(json.data);
        });
    ...
    onListDetailJson(params) {
        this.xn.dragon.post(this.superConfig.apiUrlDetail, this.buildDetailParams(params)
        ).subscribe(json => {
            this.buildRows(json.data);
        });
    }
    ...
    onDefaultSubmitEdit() {
        ...
        this.xn.dragon.post(this.superConfig.apiUrlBase + '?method=put', {
    ...
    onDefaultSubmitAdd() {
        this.xn.dragon.post(this.superConfig.apiUrlBase + '?method=post', {
