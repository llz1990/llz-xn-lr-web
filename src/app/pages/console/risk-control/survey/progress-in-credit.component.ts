...
export class ProgressInCreditComponent implements OnInit {
    ...
    size = 10; // 每页显示数据
    bankLimitAmount: number = 0; // 授信总额
    bankUsedAmount: number = 0; // 使用额度
    // 法人机构
    ...
    handleBankCredit(type?: string) {
        const params = {};
        const find = this.bankCreditList.find(x => x['selected'] === true);
        if (type === 'delete') {
    ...
    handleSelect(index: number) {
        ...
        const findIndex = this.bankCreditList.findIndex(x => x['selected'] === true);
