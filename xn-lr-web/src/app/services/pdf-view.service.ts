...
export class PdfViewService {
    ...
    // 图片初始大小
    public m_scaleValue: number = 0.6;
    ...
    public pdfToCanvas(url: string): void {
         ...
         */
        let renderPage = (num) => {
            const canvas = <HTMLCanvasElement>document.getElementById('thisCanvas');
            if (!canvas) {
                return;
            }
            ...
            self.pdfDoc.getPage(num).then((page) => {
                let viewport = page.getViewport(self.scale, self.rotate);
        ...
        let goPage = () => {
            let currentPage = (<HTMLInputElement>document.getElementById('getpageNum')).value;
        ...
        if (!this.m_init) {
            ...
            if (document.getElementById('gotoPage') !== null) {
                document.getElementById('gotoPage').removeEventListener('click', goPage);
         ...
         * 异步下载pdf
         */
        PDFJS.getDocument({ url, withCredentials: true }).then((pdfDoc_) => {
            self.pdfDoc = pdfDoc_;
