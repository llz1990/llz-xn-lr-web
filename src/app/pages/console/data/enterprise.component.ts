...
import {LogDetailModalComponent} from '../../../public/modal/log-detail-modal.component';
...
export class EnterpriseComponent implements OnInit {
    ...
    private getData(beginTime?, endTime?) {
        ...
        this.xn.api.post('/data/enterprise/all', params).subscribe(json => {
            ...
            this.chartPies = [];
            const balance = {}; // 余额
            const maxAmount = {}; // 单笔金额
            const avgTime = {}; // 平均期限
            const appName = {}; // 核心企业
            const allAmount = {}; // 收入贡献
            const proportion = {}; // 业务份额， 同累计发生额
            ...
            for (let bar of bars) { // 上面的echart样式
                ...
                for (let i = 0; i < this.list.length; i++) {
                    bar['name'].push(this.list[i][appName['index']]);
                    bar['value'].push(this.list[i][bar['index']]);
                }
            ...
            for (let bar of barpies) { // 下面的echart样式
                ...
                for (let i = 0; i < this.list.length; i++) {
                    bar['name'].push(this.list[i][appName['index']]);
                    bar['value'].push({
                        value: this.getPiesValue(this.list[i][bar['index']]),
                        name: this.list[i][appName['index']]
                    });
                }
    ...
    computeTotal(data) {
        ...
        console.log('all: ', data.map(v => v[averageDay[0]] * v[count[0]]));
        after[averageDay[0]] = this.computeAverageDay(data, averageDay, count, afterComputes); // 平均期限

        console.log('afterComputes: ', after);
    ...
    private initDate() {
        $(this.configDate.nativeElement).daterangepicker({
            ...
            console.log('日期开始: ' + start.format('YYYY-MM-DD') + ' 结束： ' + end.format('YYYY-MM-DD'));
