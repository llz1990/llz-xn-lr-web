...
export class DragonSpecialuploadComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.ctrl.valueChanges.subscribe(x => {
            let ctrlArr = this.ctrl_personList.value ? JSON.parse(this.ctrl_personList.value) : [];
            let fileType = selectType[this.selectValue];
            let isOkFlag = this.allFiles['factoringPayConfirmYyj'].length >0 || this.allFiles['pdfProjectFiles'].length>0;
            ctrlArr.map((item, index) => {
                item[fileType] = x || '';
