...
export class FeeComponent implements OnInit {
    ...
    theadText = ['核心企业 ID', '核心企业名称', '核心企业类型', '服务期限（天）', '平台服务费率（%）', '有效期'];
    ...
    onUploadExcel(e) {
        ...
        // 上传excel
        this.xn.api.upload('/llz/platformpay/excel_upload', fd).subscribe(
            res => {
    ...
    private initEditData() {
        this.xn.api
            .post('/llz/platformpay/check_data', {
    ...
    private checkChanged(originData, newData = []) {
        ...
        resData.forEach(item => {
            if (data.findIndex(y => item.appId === y.appId) >= 0) {
                ...
                const temp = data.find(x => x.appId === item.appId && x.serviceDays === item.serviceDays);
                keys.forEach(key => {
                    if (
                        temp &&
                        temp[key] &&
                        item[key] &&
                        temp[key].toString() !== item[key].toString()
                    ) {
                        // 修改
                        item[
