...
export class FinancingBank7Flow implements IFlowCustom {
    ...
    preSubmit(svrConfig: any, formValue: any): Observable<any> {
        ...
        if (
            ...
            svrConfig.actions.length === 0
        ) {
            return Observable.of(null);
        }

        const lastAction = svrConfig.actions.reverse().find(x => x.procedureId = 'operate' && x.procedureName === '银行初审');

        const nosignContracts = lastAction.checkers.find(
            x => x.checkerId === 'nosignContracts'
        );
        if (
            !nosignContracts ||
            JSON.parse(nosignContracts.data).type === 'offline'
        ) {
            return Observable.of(null);
        }
        ...
        return this.xn.api
            ...
            .map(json => {
                ...
                json.data.flowId = 'financing_bank7';
