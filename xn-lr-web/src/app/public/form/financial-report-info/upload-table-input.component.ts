...
export class UploadTableInputComponent implements OnInit {
    ...
    // 默认项
    public defaultCheckers = <CheckersOutputModel[]>[
        {
            checkerId: 'dateRange',
            required: 1,
            type: 'quantum1',
    ...
    ngOnInit() {
        ...
        let mainFormtype = [mainForm.value.dateRange.beginTime, mainForm.value.dateRange.endTime, mainForm.value.dateRange.numValue];
        // 添加一个formGroup
    ...
    private toValue() {
        this.mainFormList.map(mainform => {
            let b = Object.values(JSON.parse(mainform.value.dateRange));
