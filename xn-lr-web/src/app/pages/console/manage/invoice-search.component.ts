...
import { DateInputDayComponent } from '../../../public/component/date-input.component';
import { InvoiceSearchCompanyModalComponent } from '../../../public/form/hw-mode/modal/invoice-search-company-modal.component';
import { DescEditModalComponent } from '../../../public/modal/desc-edit-modal.component';
...
export class InvoiceSearchComponent implements OnInit, AfterViewInit {
    ...
    downloadTp(paramUrl) {
        ...
        a.href = `http://gateway.lrscft.com/api/zd/query/subject/download/annex?xwappid=eb2812c67d0740c292f9f9f70c31b542&subject_id=${paramUrl}`;
        a.click();
    }
    /**
     *  日期输入
    ...
    public updateInvoice(company) {
        ...
        this.xn.api.post('/custom/zhongdeng/invoice/get_invoicemain', {
            ...
            if (json.data.errcode !== 0) {
                ...
                let record_id = json.data.record_id;
                let timed = window.setInterval(() => {
                    this.xn.api.post('/custom/zhongdeng/invoice/get_invoicestatus', {
                        record_id: record_id
                    }).subscribe(data => {
                        if (data.data.errcode === 0) {
                            this.xn.msgBox.open(true, data.data.errmsg);
                            clearInterval(timed);
                        } else {
                        }
                    });
    ...
    public downList(value: string) {
        this.xn.router.navigate([`console/search/invoice-search/record`]);
    }
    ...
    downloadAnnex(subject: number) {
        this.xn.api.download('/custom/zhongdeng/invoice/download_annex',
            { subject_id: subject })
            .subscribe((con: any) => {
                this.xn.api.save(con._body, '发票附件.zip');
