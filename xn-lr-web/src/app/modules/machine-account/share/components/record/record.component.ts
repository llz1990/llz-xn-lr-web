...
import { DragonDataTable } from '../../../../dragon/common/data-table';
...
export class MachineRecordComponent implements OnInit {
    ...
    editAction(val: any) {
        ...
        if ((data.status !== 1 && data.status !== 0) || !XnUtils.getRoleExist(data.nowRoleId, this.xn.user.roles, 1)) {
            this.xn.router.navigate([`/machine-account/record/${this.flowId}/view/${data.recordId}`]);
        } else {
            this.xn.router.navigate([`/machine-account/record/${this.flowId}/edit/${data.recordId}`]);
        }
    }

    onNewRecord() {
        this.xn.router.navigate([`/machine-account/record/new/${this.flowId}`]);
    }
}
