...
import IntermediaryManagementTabConfig from '../../../dragon/common/intermediary-management';
...
export class DragonIntermediaryManagementComponent implements OnInit {
    tabConfig: any;
  ...
  */
    ...
    public singleChecked(paramItem, index) {
        if (paramItem['checked'] && paramItem['checked'] === true) {
            ...
            this.selectedItems = this.selectedItems.filter(x => x.appId !== paramItem.appId);
        } else {
    ...
    public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        ...
        }else if(paramBtnOperate.operate === "sub_intermediary_delete"){
            if(this.selectedItems && this.selectedItems.length > 0){
                let appIds = this.selectedItems.map(x => x.appId);
                paramBtnOperate.click(this.xn, appIds);
            }else{
    ...
    public handleRowClick(item, btn: ButtonConfigModel, i: number) {
        switch (btn.operate) {
            ...
            case 'sub_intermediary_modify':
                let appIds = item.appId;
                console.log(appIds);
                btn.click(this.xn, appIds);
                break;
        }
    }
}
