...
export class DragonChoseCapitalinfoComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.styleList = SelectOptions.get('productSource_vk');
    }
  ...
  */
    ...
    onOk() {
        this.modal.close();
        let capitalName = this.selectedItems.map(x => x.capitalPoolName)[0];
        let capitalId = this.selectedItems.map(x => x.capitalPoolId)[0];
        XnModalUtils.openInViewContainer(this.xn, this.vcr, DeletematerialEditModalComponent,
            { 'selectedItems': this.params, type: this.paramType, capitalName: capitalName, capitalPoolId: capitalId }).subscribe((x) => {
                console.info('x==>', x);
                if (x === null) {
