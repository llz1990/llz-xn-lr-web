...
export class OnceContractMultipeSelectModalComponent implements OnInit {
    ...
    public singelChecked(e, item, index) {
        if (item['checked'] && item['checked'] === true) {
            item['checked'] = false;
            this.selectedItems = this.selectedItems.filter(x => x[this.key] !== item[this.key]);
        } else {
            item['checked'] = true;
     ...
     */
    clearOne(head: any) {
        let clearindex = this.selectedItems.findIndex((item) => { return item[this.key] === head[this.key] });
        // head.checked = false;
        this.data.map(item => {
    ...
    */
    public handleSubmit() {
        if(this.params.checkerId === 'contractTemplate'){
            let mainContractList = this.selectedItems.filter((sel)=>{
