...
import { XnMoneyPipe } from '../../../../public/pipe/xn-money.pipe';
...
export class DragonViewContractModalComponent {
    ...
    public params: any;
    public pageTitle = '';
    private currentScale: number = .6; // 初始缩放
    type: number = 0;
    ...
    public handleSubmit() {
        if (this.mainForm.value.contractMoney.toString().includes(',')) {
            this.mainForm.value.contractMoney = this.ReceiveData(this.mainForm.value.contractMoney.toString());
        } else {
            this.mainForm.value.contractMoney = Number(this.mainForm.value.contractMoney);
        }
        if (this.mainForm.value.percentOutputValue !== '' && this.mainForm.value.percentOutputValue.toString().includes(',')) {
            this.mainForm.value.percentOutputValue = this.ReceiveData(this.mainForm.value.percentOutputValue.toString());
        } else {
            this.mainForm.value.percentOutputValue = Number(this.mainForm.value.percentOutputValue);
        }
        if (this.mainForm.value.totalReceive !== '' && this.mainForm.value.totalReceive.toString().includes(',')) {
            this.mainForm.value.totalReceive = this.ReceiveData(this.mainForm.value.totalReceive.toString());
        } else {
            this.mainForm.value.totalReceive = Number(this.mainForm.value.totalReceive);
        }
        let obj = Object.assign({}, {
            mainFlowId: this.params.mainFlowId,
            debtUnit: this.params.debtUnit || '',
            projectCompany: this.params.projectCompany || '', receive: this.params.receive || '',
            // contractFile: this.params.contractFile
        },
            this.mainForm.value);
        console.info('this===>', this.params.title);
