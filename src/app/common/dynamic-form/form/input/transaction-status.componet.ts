            ...
            if (x !== '') {
                if (this.ctrl1 !== null) {
                    if (JSON.parse(x).isProxy.toString() === '14') {
                        this.ctrl1.setValue('金地（集团）股份有限公司');
                    } else if (JSON.parse(x).isProxy.toString() === '-1') {
                        this.ctrl1.setValue('');
                    } else {
                        JSON.parse(x).isProxy.toString() === '52' ? this.ctrl1.setValue('深圳市龙光控股有限公司') : this.ctrl1.setValue('万科');
                    }
                }
            } else {
                this.ctrl1.setValue('');
        ...
        if (this.ctrl.value) {
            ...
            if (typeof (this.proxyValue) === 'number') {
                this.secondOptions = this.row.selectOptions.find(
                    x => x.value === this.proxyValue
                ).children;
            } else if (typeof (this.proxyValue) === 'string') {
                this.secondOptions = this.row.selectOptions.find(
                    x => x.value.toString() === this.proxyValue
                ).children;
            }
