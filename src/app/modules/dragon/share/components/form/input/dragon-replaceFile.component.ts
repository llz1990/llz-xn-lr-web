...
export class DragonPaymentReplaceComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        let headquarters = JSON.parse(this.row.value)[0].headquarters;
    ...
    replaceFile(item) {
        ...
        this.xn.dragon.post('/list/main/detail', obj).subscribe((res) => {
            if (obj.mainFlowId.endsWith('lg')) {
                if (res.ret === 0 && (res.data.currentStatus === "待放款"
                    || res.data.currentStatus === "已放款" || res.data.currentStatus === "已回款")) {
                    this.xn.msgBox.open(false, '不允许替换待放款、已放款、已回款状态的交易的付款确认书文件');
                    return;
                }

            } else if (obj.mainFlowId.endsWith('wk')) {
                if (res.ret === 0 && (res.data.currentStatus === "审批中" || res.data.currentStatus === "待签署合同"
                    || res.data.currentStatus === "待放款" || res.data.currentStatus === "已放款" || res.data.currentStatus === "已回款")) {
            ...
            if (res.ret === 0) {
                let paramsValue = this.fileTypeMatch[item.headquarters];
