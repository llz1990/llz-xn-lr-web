...
import { FlowProcessStatusEnum } from '../enums/public-enum';
import { XnService } from '../../../../services/xn.service';
import { PARAMETERS } from '@angular/core/src/util/decorators';
import Process from './mock-process';

@Component({
    selector: 'xn-avenger-flow-process-component',
    template: `
...
export class FlowProcessComponent implements OnInit, AfterViewInit {
    ...
    ngOnInit(): void {
        ...
        if (param.mainFlowId === undefined || param.mainFlowId === '' || param.mainFlowId === '1') {
            this.xn.avenger.post('/flow/navigation', {
                modelId: this.mainfromlist[0].modelId, // 协议流程 对应cfg目录的model.json vanke万科流程
                ntype: 1,
            }).subscribe(res => {

                this.flowProessmain = res.data;
                if (this.flowProessmain.modelId === 'vanke') {
                ...
                // console.info(this.flowProessmain, 'this.flowProcess');
                //this.localFlowProcess = this.buildRows(this.flowProessmain.nodeList);
            });
        } else {
            this.xn.avenger.post('/flow/navigation', {
                ...
                if (this.flowProessmain.modelId === 'vanke' && this.showAll === undefined) {
                    this.flowProessmain.nodeList = this.flowProessmain.nodeList.splice(0, 4);
                    this.localFlowProcess = this.buildRows(this.flowProessmain.nodeList);
                } else {
