 ...
 * All rights reserved.
 *
 * @file：contract-vanke-office-input.component.ts  type:'contract-office'
 * @summary：保理商补录合同信息[万科，金地]
 * @version: 1.0
 * **********************************************************************
...
@Component({
    selector: 'app-contract-vanke-office-input',
    templateUrl: './contract-vanke-office-input.component.html'
})
export class ContractVankeOfficeInputComponent implements OnInit {
    @Input() row: any;
            ...
            this.ctrl1.valueChanges.subscribe(x => {
                if (x !== '') {
                    if (JSON.parse(x)[0].files.contractNum !== undefined) {
                        let imgs = this.items[0].files.img;
    ...
    public onEdit(item) {
        ...
        if (this.row.flowId === 'financing_supplier6' && (headquarters === '万科' || headquarters === '雅居乐地产控股有限公司')) {
