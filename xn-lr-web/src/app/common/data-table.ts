...
export class DataTable {
    ...
    protected build(): void {
        ...
        let columns = this.buildTableColumns();
        let order = this.buildTableOrder(columns);
        ...
        this.table = element.DataTable({
            ...
            ajax: (data, callback, oSettings) => {
                this.getAction(data, callback, oSettings, DataTableUtils.buildUrl(this.config, 'get'));
            },
    ...
    protected bindEditorEvent(): void {
        ...
        if (this.config.allowCreate) {
            this.editor.on('initCreate', () => {
                for (let field of this.config.fields) {
                    let f = this.editor.field(field.name);
        ...
        this.editor.on('open', (e, mode, action) => {
            ...
            for (let field of this.config.fields) {
                ...
                $(`#DTE_Field_${field.options.memoLink.field}`).change((e) => {
                    ...
                    for (let v of SelectOptions.get(field.options.memoLink.selectOptions)) {
                        if (v.value === select) {
                            f.message(v.label);
                            return;
                        }
    ...
    preSubmit(e, data) {
        ...
        if (data.action === 'edit' || data.action === 'create') {
            for (let key in data.data) {
                ...
                let msg = DataTableUtils.verifyRequired(this.config, params);
                if (msg.length > 0) {
    ...
    protected buildTableColumns(): any[] {
        ...
        if (this.isShowViewButton || this.config.allowUpdate || this.config.allowDelete) {
            let html: string = '<a href="javascript:void(0)" class="editor_view">查看</a>';
            if (this.config.allowUpdate) {
    ...
    protected getAction(data: any, callback: Function, oSettings: any, url: string): void {
        this.xn.api.post2(url, this.buildGetActionParams(data))
            .subscribe((json) => {
                let j: any = $.fn;
                let i: any = j.DataTable.ext.internal;  // 这种写法避免WebStorm报_fnProcessingDisplay无法解析的错误
    ...
    protected modifyAction(method: any, url2: string, data: any, success: Function, error: Function): void {
        ...
        this.xn.api.post2(url, req)  // 只处理一条记录
            .subscribe((json) => {
                if (json.ret === 0) {
                    DataTableUtils.addRowId(this.config, json.data.data);
                    success(json.data);
    ...
    search(words) {
        console.log('words: ', this.options.where);
