...
import { ComfirmationSignComponent } from '../../../../../../pages/console/gemdale-mode/confirmation-sign-component';
...
export class AgencyPickerInputComponent implements OnInit {
    ...
    onSelect(selected: any[]) {
        ...
        if (this.row.name === 'vankeAgency' || this.row.name === 'longAgency') {
            XnModalUtils.openInViewContainer(this.xn, this.vcr, VankeAddAgencyModalComponent,
                { project_manage_id: this.project_manage_id, value: this.ctrl.value })
                .subscribe((v) => {
                    if (v === null) {
                        return;
                    } else {
    ...
    private setValueByFiles() {
        if (this.list.length === 0) {
            ...
            const obj = this.list.map(x => this.config[this.row.checkerId].formatLabelValue(x));
