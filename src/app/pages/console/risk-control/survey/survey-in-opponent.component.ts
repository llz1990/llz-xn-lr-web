...
export class SurveyInOpponentComponent implements OnInit {
    ...
    streamClick(type: any, table: string) {
        const find = this.info[table].find(x => x['selected'] === true);
        // 新增、修改
        const value = {
            upStream: {
        ...
        const params = {type: type, table: table, value: value[table][type], checkers: this.checkers[table]};
        if (type === 'delete') {
    ...
    handleSelect(index: number, label) {
        ...
        const findIndex = this.info[label].findIndex(x => x['selected'] === true);
