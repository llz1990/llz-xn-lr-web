 ...
 */
    ...
    // 资料待签署页面盖章
    onSign() {
        const tradeStatus = this.selectedItems.map(x => x.tradeStatus);
        let isOk = false;
        ...
        this.xn.dragon.post('/project_add_file/stamp', { mainFlowIds: this.selectedItems.map(x => x.mainFlowId) }).subscribe(con => {
            const cons = con.data;
