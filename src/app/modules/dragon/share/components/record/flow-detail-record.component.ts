...
export class FlowDetailRecordComponent implements OnInit {
   ...
   */
    public againSignCons(paramCons: any): void {
        ...
        contract = contract.filter(x => x.label === '应收账款转让协议书');
 ...
 */
    public showAuditStandard() {
        ...
        let contractTemp = JSON.parse(this.params.dealContract);


        let invoiceObj = JSON.parse(this.params.invoice);
        let invoiceArray = [];
    ...
    public checkSubprocess(paramRow: any) {
        this.xn.router.navigate([`/dragon/record/view`],
    ...
    public download() {
        ...
        if (this.xn.user.orgType === 1) {
            ...
            for (const proceId of procedureIdArr) {
                ...
                // files.push(JSON.parse(this.params.pdfProjectFiles));
            }
        } else {
        ...
        if (!!this.params.pdfProjectFiles) {
            JSON.parse(this.params.pdfProjectFiles).forEach(x => {
                files.push(x);
            });
        }
        if (files.length) {
            if (this.params.performanceFileSign !== null) {
                files.push(this.params.performanceFileSign);
            }
            // 拼接文件名
