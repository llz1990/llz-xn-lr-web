 ...
 */
@Component({
    templateUrl: './multiple-picker-modal.component.html',
    styles: [`
        table {
            table-layout: fixed;
            margin: 0;
...
export class MultiplePickerModalComponent {
    ...
    open(params: any): Observable<string> {
        ...
        // 初始化时有选中项，则打开添加按钮
        this.selectItems = [...this.selectItems, ...this.lists.filter(x => x['checked'] === true)];
        this.isAllChecked();
    ...
    onPage(event) {
        ...
        console.log('fenye:', this.data);
        // 切换页面 清除全选状态
        // 当数组中不具有checked且为false。没有找到则表示全选中。
        const check = this.data.some(x => x.checked === undefined || x.checked === false);
        if (!check) {
    ...
    search(reg) {
        ...
        if (reg.replace(/\s+/g, '') === '') {
            ...
            lists = this.lists.filter(x => x.appId && x.appId.includes(reg)
                || x.orgName && x.orgName.includes(reg)
                || x.orgNameSpelling && x.orgNameSpelling.includes(reg)
                || x.orgNameSyllable && x.orgNameSyllable.includes(reg)
    ...
    allPageChecked() {
        ...
        if (this.allCheckedStatus) {
            ...
            this.selectItems = [...this.selectItems, ...this.data]; // 全选将全部加入
        } else {
            this.data.forEach(item => item.checked = false);
            // 取消则，在数组中删除该数组所有项
            this.data.map(data => {
                this.selectItems = this.selectItems.filter(x => x[this.params.key] !== data[this.params.key]);
            });
        }
    }
    ...
    checkedSingle(item, index) {
        if (this.data[index]['checked'] && this.data[index]['checked'] === true) {
            ...
            // 如果取消此项，怎从数组中删除此项
            this.selectItems = this.selectItems.filter(x => x[this.params.key] !== this.data[index][this.params.key]);
        } else {
