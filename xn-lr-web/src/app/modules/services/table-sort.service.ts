...
export class DragonTableSortService {
    ...
    public tableSort(arr:any[], sortParam:string):any[] {
        let property = sortParam.split(",")[0];
        let sequence = sortParam.split(",")[1];
        for (let j = 0; j < arr.length - 1; j += 1) {
            // 两两比较，如果前一个比后一个大，则交换位置。
            for (let i = 0; i < arr.length - 1 - j; i += 1) {
                // 取出数组对应属性进行排序(升序)
                let code = this.compareString(arr[i][property], arr[i + 1][property]);
                if (code === 1) {
                    let temp = arr[i];
                    arr[i] = arr[i + 1];
                    arr[i + 1] = temp;
    ...
    private compareString(str1, str2) {
        ...
        let moneyRep = /^(^[1-9](\d+)?(\.\d{1,2})?$)|(^0$)|(^\d\.\d{1,2}$)$/;
        let StringLength = Math.min(str1Length, str2Length);
    ...
    private compareCharacter(Letter1, Letter2) {
        let Letter1TypeNum = this.DetermineType(Letter1);
        let Letter2TypeNum = this.DetermineType(Letter2);
        let before: any;
        let after: any;
        let flag: boolean;
        let code: number;
    ...
    private DetermineType(Letter) {
        // 判断类型
        let isNum = !isNaN(Letter);
        let isLetter = /[_a-zA-Z]/.test(Letter);
        let isChinese = !/[^\u4E00-\u9FA5]/.test(Letter);
        let typeNum: number;
        if (isNum) {
            typeNum = 1;
        } else if (isLetter) {
