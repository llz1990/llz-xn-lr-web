...
import { AvengerDataTable } from '../../../../common/avenger/data-table';
import { XnUtils } from '../../../../common/xn-utils';

class MyDataTable extends AvengerDataTable {
...
export class AvengerRecordComponent implements OnInit {
    ...
    editAction(val: any) {
        ...
        if ((data.status !== 1 && data.status !== 0) || !XnUtils.getRoleExist(data.nowRoleId, this.xn.user.roles, 1)) {
            this.xn.router.navigate([`/console/record/avenger/${this.flowId}/view/${data.recordId}`]);
        } else {
            this.xn.router.navigate([`/console/record/avenger/${this.flowId}/edit/${data.recordId}`]);
        }
    }

    onNewRecord() {
        this.xn.router.navigate([`/console/record/avenger/new/${this.flowId}`]);
    }
}
