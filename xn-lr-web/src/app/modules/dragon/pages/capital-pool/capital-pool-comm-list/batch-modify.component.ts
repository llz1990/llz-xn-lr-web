...
export class BatchModifyComponent implements OnInit {
    ...
    public isDemoList: boolean = false; // 台账标识
    public isVanke: boolean = false; // 资产管理标识
    ...
    ngOnInit() {
        ...
        this.data.mainFlowIdList = this.rows.map(x => x.mainFlowId);

        this.initChannelData();
    }

    public submit() {
        let param = this.infos.filter(x => x.checked === false).map(x => x.value);
    ...
    public bankChange(value) {
        ...
        if (selected) {
            ...
            this.accountOptions = this.bankInfo
                .filter(x => x.id === selected.id && x.bankName === selected.bankName)
                .map(x => {
                    return {
                        label: x.cardCode,
                        value: x.id,
    ...
    public accountChange(value) {
        ...
        //     .filter(x => x.channelType === selected.channelType &&
