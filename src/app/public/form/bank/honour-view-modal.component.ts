...
@Component({
    ...
    styles: [
        ...
        `.row { padding: 0 20px }`,
        `.img-container { max-height: calc(100vh - 440px); overflow-x: hidden; text-align: center; position: relative}`,
        `.img-wrapper { transition: all 0.5s ease-in-out; }`,
    ]
})
export class BankHonourViewModalComponent implements OnInit {
