...
export class VankeInvoiceViewModalComponent implements OnInit {
    ...
    kpje: any; // 开票金额
    fileImg: any; // 发票图像
    isdiaplayPic: boolean = false;
    titleclick: string = '显示详情';
    sail: string = '';
    forsail: string = '';
    detailinfo: string = '';
    degree = 0;
    moneyAlert = '';
    pageTitle: string = '查看发票信息';
    params: any[] = [];
    public total: number;
    public pageSize = 1;
    ...
    public showJdDetail(paramInvoiceNum: string) {
        if (!!paramInvoiceNum) {
            this.xn.dragon.post('/file/invoice_message', { invoiceNum: paramInvoiceNum }).subscribe(x => {
                if (x.data && x.data !== '') {
   ...
   */
    public onPage(e) {
        ...
        this.showJdDetail(this.params[e - 1].invoiceNum);
    }
