  ...
  template: `
    ...
    <form class="form-horizontal" (ngSubmit)="handleSubmit()">
      ...
      <modal-footer>
        ...
        <button type="submit" class="btn btn-success" (click)='enterCapital()'>
          {{paramButton}}
        </button>
      </modal-footer>
    </form>
...
export class DeletematerialEditModalComponent {
   ...
   */
  public handleSubmit() {
    let val = this.mainForm && this.mainForm.value ? this.mainForm.value : null;
    this.close(val);
  }
  ...
  enterCapital() {
    if (this.type === 1) {
      this.xn.dragon.post('/pool/add',
        { mainFlowIdList: this.params.map(x => x.mainFlowId), capitalPoolId: this.capitalPoolId }).subscribe(x => {
          if (x.ret === 0) {
