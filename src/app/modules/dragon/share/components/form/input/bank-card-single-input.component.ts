...
@Component({
    ...
    templateUrl: './bank-card-single-input.component.html',
    styles: [`
        .title {
            padding: 10px 0;
            font-weight: bold;
...
@DynamicForm({ type: 'bank-single', formModule: 'dragon-input' })
export class BankCardSingleInputComponent implements OnInit {
    @Input() row: any;
    ...
    public onChange(e) {
        ...
        const find = this.bankLists.find(x => x.cardCode === value);
        if (find) {
