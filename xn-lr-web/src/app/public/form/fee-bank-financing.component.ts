...
@Component({
    selector: 'xn-fee-bank-financing',
    templateUrl: './fee-bank-financing.component.html',
    styles: [
        `
            .table {
                font-size: 13px;
...
export class FeeBankFinancingComponent implements OnInit {
    @Input() row: any;
    @Input() form: FormGroup;

    theadText = ['核心企业 ID', '核心企业名称', '核心企业类型', '是否可融资', '融资可用额度', '融资期限', '年化利率'];
    ...
    onUploadExcel(e) {
        ...
        // 上传excel
        this.xn.api.upload('/llz/bank_financing/excel_upload', fd).subscribe(
            res => {
    ...
    private initEditData() {
        this.xn.api
            .post('/llz/bank_financing/check_data', {
    ...
    private checkChanged(originData, newData = []) {
        ...
        resData.forEach(item => {
            if (data.findIndex(y => item.appId === y.appId) >= 0) {
                ...
                const temp = data.find(x => x.appId === item.appId && item.discountDays === x.discountDays);
                keys.forEach(key => {
