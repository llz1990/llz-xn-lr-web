...
export class VankeEnterpoolComponent implements OnInit {
    tabConfig: any;
    ...
    public handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        ...
        if (this.selectedItems.length === 0) {
            ...
            if (paramBtnOperate.operate === 'enter-capitalpool') {
                this.enterCapitalpool(1, this.headquarters);
            } else if (paramBtnOperate.operate === 'delete-capitalpool') {
                this.enterCapitalpool(2);
            } else if (paramBtnOperate.operate === 'batch-information') {
                this.batchModify();
            } else if (paramBtnOperate.operate === 'download-file') {
                this.downloadSelectedAttach();
            } else if (paramBtnOperate.operate === 'export-file') {
                this.exportCapital();
            }
        }
    ...
    private hasSelectRow() {
        const selectedRows = this.data.filter(x => x.checked && x.checked === true);

        return !!selectedRows && selectedRows.length > 0;
    }
    private isDifferentCompany() {
        const company = XnUtils.distinctArray(this.data.filter(x => x.headquarters).map(c => c.headquarters)) || [];
...
*/
    public downloadSelectedAttach() {
        ...
        this.xn.dragon.post('/project_manage/file_contract/down_file_title', { project_manage_id: '' }).subscribe(x => {
            if (x.ret === 0) {
    ...
    public onPage(e?) {
        ...
        // 页面配置
        this.currentTab = this.tabConfig.tabList.find(x => x.value === this.label); // 当前标签页
    ...
    enterCapitalpool(paramType: number, headquarters?: string) {
        if (paramType === 1) {
            if (this.fitProject !== '') {
                ...
                headquarters = headquarters === '万科' ? '万科企业股份有限公司' : headquarters;
                XnModalUtils.openInViewContainer(this.xn, this.vcr, DragonChoseCapitalinfoComponent,
                    { 'selectedItems': this.selectedItems, type: paramType, headquarters: headquarters }).subscribe((x) => {
                        console.info('xxx=>', x);
                        if (x === null) {
                            return;
                        } else {
    ...
    public checkAll(e) {
        ...
        this.selectedReceivables = this.selectedItems.reduce((accumulator, currentValue) => { return accumulator + Number(currentValue['receive'] ? currentValue['receive'] : 0); }, 0).toFixed(2);
        this.selectedPayableAmounts = this.selectedItems.reduce((accumulator, currentValue) => { return accumulator + Number(currentValue['changePrice'] ? currentValue['changePrice'] : 0); }, 0).toFixed(2);
    }
    ...
    private buildCondition(searches) {
        ...
        for (let i = 0; i < searches.length; i++) {
            ...
            if (searches[i]['checkerId'] === 'headquarters' && this.isFirst) {
                if (this.fitProject !== '') {
                    obj['value'] = this.headquarters === '万科企业股份有限公司' ? '万科' : this.headquarters;
                    this.arrObjs[searches[i]['checkerId']] = obj['value'];
                    this.isFirst = false;
                } else {
            ...
            for (const search of this.searches) {
                if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
                    if (search.checkerId === 'tradeStatus') {
                        ...
                        params['selectBank'] = String(val.status) !== '0' && !!val.status ? String(val.status) : '0';
                    } else {
                        params[search.checkerId] = this.arrObjs[search.checkerId];
                    ...
                    if (search.checkerId === 'headquarters') {
                        this.headquarters = this.arrObjs[search.checkerId];
                        params[search.checkerId] = this.arrObjs[search.checkerId];
                    }
