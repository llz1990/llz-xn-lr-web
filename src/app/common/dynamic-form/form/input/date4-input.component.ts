...
export class Date4InputComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        let $configDate = $(this.configDate.nativeElement);
        let $alertInfo = $(this.alertInfo.nativeElement);
        let regs = /^(?:19|20)[0-9][0-9](?:(?:0[1-9])|(?:1[0-2]))(?:(?:[0-2][1-9])|(?:[1-3][0-1]))$/g;
        ...
        $configDate.on('change', (e) => {
            e.preventDefault();
            let reg = /^(?:19|20)[0-9][0-9](?:(?:0[1-9])|(?:1[0-2]))(?:(?:[0-2][1-9])|(?:[1-3][0-1]))$/g;
            if (String($configDate.val()) !== '' && !(reg.test($configDate.val()))) {
                $configDate.addClass('not-invalid');
                $alertInfo.html('请输入正确有效的时间格式，格式为20200101');
    ...
    private initDate() {
        ...
        $(this.configDate.nativeElement).daterangepicker({
            ...
            minDate: this.row.name === 'valueDate' ? new Date() : null,
            locale: {
                format: "YYYYMMDD",
                separator: "",
            ...
            $configDate.val(picker.startDate.format('YYYYMMDD'));
            _this.ctrl.setValue(picker.startDate.format('YYYYMMDD'));
            $alertInfo.html('');
        });
    }
