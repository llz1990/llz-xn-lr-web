...
import { TabListOutputModel, SubTabListOutputModel, ButtonConfigModel } from '../../../../config/list-config-model';
...
import { CapitalPoolAlertRatioModalComponent } from '../../share/modal/capital-pool-alert-ratio-modal.component';
import { VankechoseTemplateModalComponent } from '../../share/modal/vanke-chose-contractTemplate-modal.component';
declare const moment: any;

@Component({
    selector: 'vanke-project-management-component',
    templateUrl: `./vanke-project-management.component.html`,
    styles: [`
    .item-box {
        position: relative;
        display: flex;
...
export class VankeProjectmanageComponent implements OnInit {
    tabConfig: any;
    ...
    public handleRowClick(item: any, btn: ButtonConfigModel, i: number) {
        if (btn.operate === 'confirm_receivable') { // 基础资产
            ...
            this.xn.router.navigate(['/dragon/vanke/projectPlan-management'], {
                queryParams: {
                    title: title + item.project_manage_id + '>-' + item.headquarters,
                    projectId: Number(item.project_manage_id),
                    headquarters: item.headquarters,
                    paging: this.paging,
                    defaultValue: this.defaultValue
 ...
 * 设置中介机构
 */
    public intermediaryAgencyForm(params?: { value: number }) {
        const checkers = [
            {
  ...
  */
                ...
                if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
                    if (search.checkerId === 'headquarters') {
                        if (this.arrObjs[search.checkerId] === '万科') {
                            params[search.checkerId] = '万科企业股份有限公司';
    ...
    setUpProject(types: number, paramsInfo?: any) {
        ...
        if (paramsInfo !== undefined && paramsInfo.headquarters === '万科企业股份有限公司') {
            paramsInfo.headquarters = '万科';
        }
        const checkers = [{
            title: '项目名称',
            checkerId: 'projectName',
        ...
        },
        {
            ...
            type: 'date',
            required: false,
            value: paramsInfo !== undefined ? moment(paramsInfo.passTime).format('YYYY-MM-DD') : ''
        },
        {
            title: '交易场所',
            checkerId: 'dealSite',
        ...
        },
        {
            ...
            type: 'select',
            required: 1,
            options: { ref: 'enterprise_dragon1' },
            value: paramsInfo !== undefined ? paramsInfo.headquarters : ''
        },
        {
            title: '项目类型',
        ...
        {
            title: '专项计划编号',
            checkerId: 'specificPlanNum',
            type: 'text',
            required: false,
            value: paramsInfo !== undefined ? paramsInfo.specificPlanNum : ''
        },
        {
            title: '预期期数',
            checkerId: 'predictPeriods',
            type: 'text',
            required: false,
            value: paramsInfo !== undefined ? paramsInfo.predictPeriods : ''
        },
        ];
        let params = {
            checker: checkers,
            ...
            .subscribe((v) => {
                ...
                if (v === null) {
                    ...
                    if (types === 2) {
                        ...
                        v.projectType = Number(v.projectType);
                        v.headquarters = v.headquarters === '万科' ? '万科企业股份有限公司' : v.headquarters;
