...
export class DragonMfileTableInputComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.ctrl_file.valueChanges.subscribe((f)=>{
            if(f!==''){
                let ctrVal = XnUtils.parseObject(this.allFiles[QrsTypeEnum[f]], []);
    ...
    public onBeforeSelect(e) {
        if(!this.ctrl_type.value || !this.ctrl_headquarters.value || !this.ctrl_file.value){
            e.stopPropagation();
            e.preventDefault();
            let msg = `请先选择${!this.ctrl_type.value?'业务类型':!this.ctrl_headquarters.value?'总部公司':'文件类型'}`;
    ...
    private setValueByOcrRes() {
        ...
        let postObjs = {	
            ...
            fileType: fileType    //Number(this.ctrl_file.value) 3=付款确认书（万科） 1=付款确认书（总部致保理商）（龙光） 2=付款确认书（总部致券商）（龙光）  
        };
        ...
        this.xn.api.avenger.post2('/sub_system/ocr/get_ocr_result', postObjs).subscribe(res => {
            ...
            if(res.ret === 0){
                ...
                if(res.data.status === 3){
                    ...
                    let ocrResult = res.data.orcList;
