...
import { ContractVankeSupplementModalComponent } from '../../../public/modal/contract-vanke-supplement-modal.component';
...
export class ComfirmInformationIndexComponent implements OnInit {
    ...
    private reSignContracts(btn) {
        this.xn.loading.open();
        this.xn.api.post(btn.get_api, { mainFlowIds: this.selectedItems.map(x => x.mainFlowId) })
            .subscribe(con => {
    ...
    private initiateSupplementalAgreement(btn) {
        this.xn.loading.open();
        this.xn.api.post(btn.get_api, { mainIdList: this.selectedItems.map(x => x.mainFlowId) })
            .subscribe(con => {
    ...
    private batchSign(btn) {
        this.xn.api.post(btn.value, { mainFlowIds: this.selectedItems.map(x => x.mainFlowId) })
            .subscribe(con => {
                const cons = con.data.contractList;
                cons.forEach(element => {
                    if (!element['config']) {
    ...
    private confirmReceivable(btn) {
        ...
        this.xn.api.post(btn.value,
            ...
            .subscribe(next => {
                if (next && next.data && next.data.length) {
                    const mains = next.data.map(x => x.mainFlowId);
