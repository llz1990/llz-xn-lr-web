...
export class ElementDragDirective implements OnInit {
    ...
    public isDown: boolean = false; // 判断该元素是否被点击了
    public disX: number; // 记录鼠标点击事件的位置 X
    public disY: number; // 记录鼠标点击事件的位置 Y
    private totalOffsetX: number = 0; // 记录总偏移量 X轴
    private totalOffsetY: number = 0; // 记录总偏移量 Y轴

    private defaultX: number = 30; // 初始偏移量 X轴
    private defaultY: number = 30; // 初始偏移量 Y轴
