 ...
 */
@Component({
    selector: 'app-risk-warning-collection-reminder-index2',
    template: `
        <section class="content">
            <div style="padding: 10px">
                <div class="form-group clearfix">
                    <div style="float: left;margin-top: 5px">距离到期日：</div>
                    <div style="padding: 0;float: left;width: 150px" *ngFor="let row of shows">
                        <xn-input [form]="mainForm" [row]="row"></xn-input>
                    </div>
                    <div style="float: left;margin-left: -1px">
                        <button type="button" style="border-left: none;"
                                class="btn btn-default" (click)="searchMsg()">查询
                        </button>
                    </div>
                </div>
...
export class CollectionReminderIndex2Component implements OnInit {
    @Input() urlData: any;
    @Input() tip: any;
    // 时间选择
    ...
    postUrl = '/llz/risk_warning/warning/collect_tips';
    ...
    calcTotal() {
        if (this.info.length) {
            const arr = this.info.map(x => x.contractAmount);
            if (arr.length) {
    ...
    pageChange(page) {
        ...
        // 如果没有该字段,就添加默认天数
        if (!params.hasOwnProperty('leftTime')) {
            params['leftTime'] = 7;
        }
        // 前段进行分页
    ...
    private splitPage(url, params, start, length): Observable<any> {
        if (this.resCache.has('data')) {
            ...
            return this.xn.api.post(url, params).map(x => {
