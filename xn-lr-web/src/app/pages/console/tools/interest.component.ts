...
export class InterestComponent implements OnInit {
    ...
    calculate() {
        ...
        let billAmount = parseFloat(money.replace(/[,]/g, ''));
        let startDate = Date.parse(this.StartDate.nativeElement.value) || '';
        let expireDate = Date.parse(this.ExpireDate.nativeElement.value) || '';
        let useRate = this.UseRate.nativeElement.value;
        let serviceRate = this.ServiceRate.nativeElement.value;
        let platfromRate=this.platformServiceSate.nativeElement.value;//平台服务费取值
        let PlatServiceRateData=this.PlatServiceRateData.nativeElement.value;
        ...
        if (billAmount && startDate && expireDate && useRate && serviceRate) {
            ...
            let calcDay = (Number(expireDate) - Number(startDate)) / 1000 / 60 / 60 / 24; // 计息天数
            let timeRate = (Number(expireDate) - Number(startDate)) / (1000 * 86400 * 360); // 融资期限
            let useRateData = Number(((useRate / 100) * timeRate * billAmount).toFixed(2)); // 使用费
            let serviceRateData = Number(((serviceRate / 100) * timeRate * billAmount).toFixed(2));  // 服务费
            let platserviceRateData=Number((billAmount*(platfromRate/100)*Math.ceil(calcDay/30)).toFixed(2));//平台服务费
            let useServiceData = (useRateData + serviceRateData+platserviceRateData).toFixed(2); // 使用费+服务费+平台服务费
            let totaldata = (billAmount - Number(useRateData)-serviceRateData-platserviceRateData).toFixed(2); // 本金-使用费-服务费-平台服务费
