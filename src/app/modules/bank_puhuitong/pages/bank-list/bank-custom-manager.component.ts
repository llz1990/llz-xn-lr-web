...
import { BankPdfSignModalComponent } from '../../share/modal/pdf-sign-modal.component';
...
export class BankPuhuiTongComponent implements OnInit {
    ...
    public singelChecked(e, item, index) {
        if (item['checked'] && item['checked'] === true) {
            item['checked'] = false;
            this.selectedItems = this.selectedItems.filter(x => x.appId !== item.appId);
        } else {
            item['checked'] = true;
    ...
    public showContract(con) {
        let contractLsit = JSON.parse(con);
    ...
    handleHeadClick(paramBtnOperate: ButtonConfigModel) {
        if (this.selectedItems.length === 0) {
            ...
            if ((paramBtnOperate.operate === 'openPush'
                ...
                let flag = paramBtnOperate.operate === 'openPush' ? 1 : 0;
                let appIds = this.selectedItems.map(x => x.appId);
                let msgStr = flag ? '开启推广' : '关闭推广';
                this.xn.avenger.post(paramBtnOperate.post_url, { appIds: appIds, pushStatus: flag }).subscribe(x => {
                    if (x.ret === 0) {
            ...
            for (const search of this.searches) {
                if (!XnUtils.isEmpty(this.arrObjs[search.checkerId])) {
                    if (search.checkerId === 'day') {
                        let data = JSON.parse(this.arrObjs[search.checkerId]);
