...
@Component({
    ...
    <div class="input-group">
        <ng-container *ngIf="row.checkerId!=='contractTemplate'">
            <select class="form-control xn-input-font" [value]="selectValue" [disabled]="true">
...
export class DragonMultiplePickerShowComponent implements OnInit,AfterViewInit,OnDestroy {
    ...
    ngOnInit() {
        ...
        if(!!this.row.data && JSON.parse(this.row.data) && this.judgeDataType(JSON.parse(this.row.data)) && JSON.parse(this.row.data)[0] === '通用'){
            this.selectValue = '通用';
