 ...
 * 1.0                 wangqing          查看文件方法修改     2019-09-05
 * **********************************************************************
 */
...
export class MfilesViewModalComponent implements OnInit {
        ...
        if (this.params && this.params.length > 0) {
            this.params.forEach(x => {
                ...
                if (this.fileType === 'img') {
                    this.fileSrc.push({picType: 0, url: this.xn.file.dragonView(x)});
                } else {
                    let self = this;
                    new Promise((resolve, reject) => {
                        self.fileSrc.push({picType: 1, url: self.xn.file.dragonView(x)});
                    }).then(() => {
                        setTimeout(() => {
                            self.pdfViewService.pdfToCanvas(self.xn.file.dragonView(x));
                        }, 0);
                    })
                    // this.fileSrc.push({picType: 1, url: this.xn.file.dragonView(x)});
                    // let self = this;
                    // setTimeout(() => {
                    //     self.pdfViewService.pdfToCanvas(self.xn.file.dragonView(x));
                    // }, 0);
