 ...
 */
@Component({
    selector: 'xn-newvanke-data-content',
    templateUrl: './data-content-newvanke.component.html',
    styles: [`
        .table-display tr td {
            width: 200px;
...
@DynamicForm({ type: 'data-content1', formModule: 'dragon-input' })
export class NewVankeDataContentComponent implements OnInit, OnChanges, OnDestroy {
    ...
    ngOnInit() {
        ...
        // 暂存的总部公司
        this.type = this.localStorageService.caCheMap.get('headquarters') || '万科企业股份有限公司';
    ...
    onScroll($event: Event) {
        ...
        let fixedColumn = $(".tables").find(".head-height tr td:nth-child(1),.table-height tr td:nth-child(1)");
        ...
        this.xn.api.dragon.upload(this.heads[this.type].excel_up_url, fd).subscribe(json => {
            ...
            if (json.type === 'complete') {
                if (json.data.ret === 0) {
                    ...
                    if (msg.indexOf(';') !== -1) {
                        let uniqueStr = XnUtils.distinctArray(msg.split(']')[1].replace(/\s*/g,"").split(';'));
