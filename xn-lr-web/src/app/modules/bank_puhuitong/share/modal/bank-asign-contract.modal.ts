...
@Component({
    ...
    templateUrl: './bank-asign-contract.modal.html',
    styles: [
        `
            .pdf-container {
                width: 100%;
...
export class BankFinancingContractModalComponent implements OnInit {
    ...
    open(paramContracts: any): Observable<string> {
        // this.isReadonly = paramContracts.readonly || false;
        // 默认第一个激活
        this.activeCode = 0;
    ...
    public onSave() {
        ...
        this.xn.api.avenger.download('/file/downFile', {
            files: this.contractLists,
        }).subscribe((v: any) => {
            this.xn.avenger.save(v._body, `${orgName}合同文件.zip`);
        });
    }
    ...
    private onSign(paramContract: any) {
        ...
        setTimeout(() => {
            ...
            this.xn.avenger.post3('/file/contractUpload', {
                id: paramContract.id,
    ...
    private displaySign(paramCurrentContract: any): void {
        ...
        this.xn.api.avenger
            ...
            .subscribe(res => {
                try {
                    const ret = this.ocx.SZCA_LoadFromString(res.data); // 加载文件流
                    if (ret !== 0) {
                        alert(this.ocx.SZCA_GetErrMsg(ret));
                        return;
                    }
