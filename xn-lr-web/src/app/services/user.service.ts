...
export class UserService {
    ...
    private todoCount = new Subject<number>();
    ...
    login(account: string, code: string): Observable<any> {
    ...
    tmpLogin(appId: string, orgType: string, tmpToken: string): Observable<any> {
    ...
    logoutNoRedirect() {
        ...
        window.sessionStorage.removeItem('appNature');
        window.sessionStorage.removeItem('isAdmin');
        window.sessionStorage.removeItem('isPlatformAdmin');
        window.sessionStorage.removeItem('env');
        window.sessionStorage.removeItem('isInvoice');
        window.sessionStorage.removeItem('customFlowIds');
        window.sessionStorage.removeItem('appType');
        window.sessionStorage.removeItem('enterpriseType');
    ...
    redirectWhenLogin(): void {
        console.log("redirectUrlWhenLogin",this.redirectUrlWhenLogin);
    ...
    redirectConsole(): void {
        let res = this.cache.roles.filter(function(item,index,array){
    ...
    getTodoCount() {
        return this.api.post('/user/todo_count', {}).subscribe((json) => {
            this.updateTodoCount(json.data.count);
        });
    }

    updateTodoCount(count: number) {
        this.todoCount.next(count);
    }
