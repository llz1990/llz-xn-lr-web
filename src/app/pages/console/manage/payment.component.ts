...
import { RatesDateStartModalComponent } from '../../../public/modal/rates-date-start-modal.component';
...
export class PaymentComponent implements OnInit {
    ...
    type = 'pay_no_do';
    lastType = '';
    completeMoneystyle: boolean = true;
    ...
    ViewContract(paramsFile: any[]) {
        let paramsFiles = JSON.stringify(paramsFile);
        ...
        XnModalUtils.openInViewContainer(this.xn, this.vcr, EditInfoModalComponent, params).subscribe(v => {
            ...
            if (v.cashierDown === '2') { // 直接下载
                ...
                this.xn.api.download('/mdz/down_file/load_financing_wf',
                    { mainFlowIdList: json.map(j => j['mainFlowId']), maxMoney: v.maxMoney * 10000, maxNumber: v.maxNumber })
                    .subscribe((next: any) => {
                        this.xn.api.save(next._body, '出纳下载表格(潍坊模板).xlsx');
                        // 上传下载次数
                        this.xn.api.post('/mdz/main/finance_load_times',
                            { mainFlowIds: json.map(j => j['mainFlowId']) }).subscribe(() => {
                ...
                this.xn.api.download('/mdz/down_file/load_financing_pf',
                    { mainFlowIdList: json.map(j => j['mainFlowId']) })
                    .subscribe((next: any) => {
                ...
                this.xn.api.post('/mdz/down_file/record_cn',
                    {
                        mainFlowIds: json.map(j => j['mainFlowId']),
    ...
    accountingDown() {
        this.localStorageService.setCacheValue('accounting_load_mainFlowIds',
            this.payDoSelectedDownload.map(x => x['mainFlowId'])); // 暂存选中的值
    ...
    completeMoney() {
        this.xn.api.post('/custom/vanke_v5/project/finish_payment',
            { 'mainIds': this.payDoSelectedDownload.map(x => x['mainFlowId']) }).subscribe(x => {
                if (x.ret === 0) {
                    this.onPage({
                        page: this.pageIndex,
                        pageSize: this.pageSize
    ...
    backPreStep() {
        ...
        this.xn.api.post('/ljx/pay_manage/pay_do_back', { mainFlowIds: mainFlowIds }).subscribe(x => {
            if (x.ret === 0) {
    ...
    // 获得应付账款金额汇总
    totalAmount() {
        let dataInfo = JSON.parse(JSON.stringify(this.data.filter(item => item.checked)));
