  ...
  */
    ...
    choseAgency() {
        ...
        this.datalist01.forEach((x) => {
            this.ceilList.push({ appId: x.investTierName, appName: x.investTierName });
        });
        const checkers = [
            {
        ...
        XnModalUtils.openInViewContainer(
            ...
            if (isSame.length === 0) {
                this.dataAgency.push({
                    purchaseMoney: this.ReceiveData(data.purchaseMoney),
                ...
                this.xn.msgBox.open(false, '投资机构+层级不能重复');
            }
        });

    }
    // 修改中介机构
    changeAgency(paramInfo, paramIndex) {
        this.ceilList = [];
        this.datalist01.forEach((x) => {
            this.ceilList.push({ appId: x.investTierName, appName: x.investTierName });
        });
        ...
        XnModalUtils.openInViewContainer(
            ...
            this.dataAgency[paramIndex].invest_tier_id = this.datalist01.filter(x => x.investTierName === data.investTierName)[0].invest_tier_id;
            this.cdr.markForCheck();
    ...
    // 修改层级
    changeRece(paramInfo, paramIndex) {
        const checkers = [
